...
const readerShell=fs.readFileSync(path.join(root,'..','reader.html'),'utf8'),sandbox={window:{}};
vm.runInNewContext(fs.readFileSync(path.join(root,'..','scripts','target-data.js'),'utf8'),sandbox);
const reader=readerShell+sandbox.window.WH_ARMY_BOOK_TARGETS.html;
const runtimeVersions=JSON.parse(fs.readFileSync(path.join(root,'..','..','shared','runtime-asset-versions.json'),'utf8'));
...
assert.ok(!redirect.includes("destination.searchParams.delete('view')"),'Redirect must preserve the responsive view mode');
...
assert.ok(/\.\/scripts\/roster-enhancements\.js\?v=\d+/.test(reader)&&/\.\/scripts\/roster-filter\.js\?v=\d+/.test(reader),'Responsive reader does not load the common roster path');
