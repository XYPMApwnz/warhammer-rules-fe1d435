(async function(){
  ...
  viewSwitch?.addEventListener('click',()=>{const active=navigation.active;let route='index.html',anchor='start';for(let node=navigation.byId.get(active)?.node;node;node=node.parentElement?.closest('[data-nav-id]')){const id=node.dataset.navId;if(id==='start'){anchor=active;break;}if(id==='updates'){route='updates.html';anchor=active;break;}if(id==='core-rules'){route='army-rules.html';anchor=active;break;}if(id.startsWith('detachment-')){route=id.slice(11)+'.html';anchor=active;break;}if(id.startsWith('unit-')){const canonical=document.getElementById(id)?.dataset.rosterCanonicalId||id;route=canonical.slice(5)+'.html';anchor=canonical;break;}}const destination=new URL('./mobile/'+route,location.href);destination.search=new URLSearchParams(location.search).toString();destination.hash=anchor;viewSwitch.href=destination.href;});
