(async function(){
  ...
  function initCawlColumns(){
    if(!matchMedia('(min-width:801px)').matches)return;
    const card=document.getElementById('unit-belisarius-cawl'),grid=card?.querySelector(':scope > .ds-main-grid'),arsenal=grid?.querySelector(':scope > .ds-arsenal'),support=grid?.querySelector(':scope > .ds-support');
    if(!arsenal||!support)return;
    card.querySelectorAll(':scope > .unit-part, :scope > .related-rules-trigger').forEach(section=>arsenal.append(section));
    const continuation=card.querySelector(':scope > .ds-abilities-continuation');if(continuation)support.append(continuation);
  }
  ...
  function initRelatedRules(){
    ...
    async function open(current,state={}){
      ...
      if(!content)try{
        ...
        const detachmentSections=sections.filter(section=>section.dataset.detachment!=='core'),choices=[...(!rosterMode?[['all','All Detachments']]:[]),...detachmentSections.map(section=>[section.dataset.detachment,section.querySelector('h2').textContent])];detachment=rosterMode?'all':(choices[0]?.[0]||'all');
      ...
      if(!rosterMode&&state.detachment&&sections.some(section=>section.dataset.detachment===state.detachment))detachment=state.detachment;filter();layer.querySelector('.related-rules-dialog').scrollTop=state.scrollTop||0;modal.focusFirst();
    }
  ...
  requestAnimationFrame(()=>requestAnimationFrame(initCawlColumns));
