(function(root){
  ...
  function applyEnhancement(card,item,entry,state,owner){
    ...
    switch(id){
      ...
      case'enhancement-sanctified-ordnance':ranged.forEach(row=>numberCell(row,'Range',6));break;
  ...
  const projectAttachments=(card,state)=>{
    ...
    if(unitId(state.current)==='unit-kastelan-robots'&&ids.has('unit-cybernetica-datasmith'))modelStat(card,'T',1);
  };
  ...
  const projectGameEffects=(roster,unit,context={})=>{
    ...
    const ability=(id,title,summary,source)=>add(id,'ability',id,'grant',{title,summary},source),canonicalReference=(kind,id,source)=>add(`canonical-${id}`,'ability',id,'reference',{canonicalReference:{kind,id},...(kind==='ability'?{canonicalAbilityId:id}:{}),state:'reference'},source),canonicalAbility=(id,source)=>canonicalReference('ability',id,source),canonicalEnhancement=(id,source)=>canonicalReference('enhancement',id,source),stat=(id,targetId,operation,value,source)=>add(id,'stat',targetId,operation,operation==='add'?{delta:value}:{to:value},source),weapon=(id,scope,operation,detail,source)=>add(id,'weapon',scope,operation,detail,source);
    if(!state.current)return output;
    const ids=new Set(state.members.map(unitId));
    if(state.attached)for(const sourceUnit of state.members){const sourceId=unitId(sourceUnit),source=origin('explicit-attachment',`datasheet-${sourceId.replace(/^unit-/,'')}`,sourceUnit.id,'explicit-attachment');if(sourceId==='unit-skitarii-marshal'&&sourceUnit.id!==state.current.id)canonicalAbility('datasheet-control-edict',origin('explicit-attachment','datasheet-control-edict',sourceUnit.id,'explicit-attachment'));if(sourceId==='unit-tech-priest-dominus')ability('lord-of-the-machine-cult',`Feel No Pain ${state.unitKeywords.has('ELECTRO-PRIESTS')?4:5}+`,`Models represented by this roster unit have Feel No Pain ${state.unitKeywords.has('ELECTRO-PRIESTS')?4:5}+.`,source);if(sourceId==='unit-tech-priest-manipulus')weapon('galvanic-field','all','grant-tag',{tag:'LETHAL HITS',termId:'core-lethal-hits'},source);if(sourceId==='unit-technoarcheologist')stat('technoarcheologist-oc','OC','add',1,source);}
    ...
    if(ids.has('unit-fulgurite-electro-priests')){const character=state.members.find(member=>member.id!==state.current.id&&unitMeta(member).keywords?.includes('CHARACTER'));if(character)canonicalAbility('datasheet-electro-infusion',origin('explicit-attachment','datasheet-electro-infusion',character.id,'explicit-attachment'));}
