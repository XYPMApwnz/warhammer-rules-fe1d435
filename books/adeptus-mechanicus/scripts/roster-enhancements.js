(function(root){
  ...
  const projectGameEffects=(roster,unit,context={})=>{
    ...
    const ability=(id,title,summary,source)=>add(id,'ability',id,'grant',{title,summary},source),canonicalAbility=(id,source)=>add(`canonical-${id}`,'ability',id,'reference',{canonicalAbilityId:id,state:'reference'},source),stat=(id,targetId,operation,value,source)=>add(id,'stat',targetId,operation,operation==='add'?{delta:value}:{to:value},source),weapon=(id,scope,operation,detail,source)=>add(id,'weapon',scope,operation,detail,source);
