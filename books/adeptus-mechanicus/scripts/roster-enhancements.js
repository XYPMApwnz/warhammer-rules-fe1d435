(function(root){
  ...
  const projectGameEffects=(roster,unit,context={})=>{
    ...
    if(unitId(state.current)==='unit-kastelan-robots'&&ids.has('unit-cybernetica-datasmith')){stat('aegis-protocol-toughness','T','add',1,origin('explicit-attachment','datasheet-battle-protocols',state.current.id,'explicit-attachment'));ability('aegis-protocol','Aegis Protocol','This unit starts the battle in Aegis Protocol. A later Command phase selection can replace it.',origin('explicit-attachment','datasheet-battle-protocols',state.current.id,'explicit-attachment'));}
    if(ids.has('unit-fulgurite-electro-priests')&&state.members.some(member=>unitMeta(member).keywords?.includes('CHARACTER')))ability('electro-infusion','Electro-Infusion','While led, subtract 1 from Wound rolls for attacks that target this unit.',origin('explicit-attachment','datasheet-electro-infusion',state.current.id,'explicit-attachment'));
  ...
  function decorate(card,roster,units,context={}){
    ...
    if(Array.isArray(context.projectedEffects)){applyProjectedEffects(card,context.projectedEffects);return;}
