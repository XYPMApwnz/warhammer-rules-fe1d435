(function(){
  ...
  if(requestedGroup){const card=requestedGroup.card,currentTarget=document.getElementById(location.hash.slice(1)),sectionTarget=currentTarget&&card.contains(currentTarget)?currentTarget:null,target=sectionTarget||card,url=new URL(location.href);url.hash=target.id;history.replaceState(history.state,'',url);window.addEventListener('load',()=>{let frames=0;const restore=()=>{if(window.DG_APP?.navigation){document.querySelector(`[data-nav-target="${CSS.escape(card.id)}"]`)?.click();if(sectionTarget){url.hash=sectionTarget.id;history.replaceState(history.state,'',url);window.DG_APP.navigation.navigate(card.id,sectionTarget);}return;}if(frames++<300)requestAnimationFrame(restore);};requestAnimationFrame(restore);},{once:true});}
}());
