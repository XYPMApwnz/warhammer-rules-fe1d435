...
check('unit art remains decorative and layout-stable on Desktop and Phone',(html.match(/<figure class="unit-art-background" aria-hidden="true"/g)||[]).length===34&&(html.match(/<img src="\.\/assets\/unit-images\/[^"]+"[^>]+alt=""/g)||[]).length===34&&unitImagePages.every(page=>/<img src="\.\.\/assets\/unit-images\/[^"]+"[^>]+alt=""/.test(page)));
...
check('unconditional whole-unit Enhancements project onto the exact bearer',!read('scripts/roster-enhancements.js').includes("if(['ranged-range-6','ranged-range-6-strength-1','skitarii'].includes(effect))return")&&read('scripts/roster-enhancements.js').includes("if(effect==='skitarii')return addKeyword(card,'SKITARII')")&&read('scripts/roster-enhancements.js').includes("if(effect.startsWith('ranged-'))"));
