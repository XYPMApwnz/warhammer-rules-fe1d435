...
check('responsive reader popup uses canonical Mechanicus glossary and shared renderers',/glossary\/generated\/glossary\.en\.js\?v=/.test(html)&&appSource.includes("WH40K_GLOSSARY?.forBook('adeptus-mechanicus')")&&canonicalPopupRuntime.includes('WHPopupContent')&&appSource.includes('WHGlossaryAutolink?.apply'));
...
check('Compatible Rules runtime uses only the generated matrix',appSource.includes('generated/compatible-rules.json')&&!appSource.includes('AMRelatedRules')&&!fs.existsSync(path.join(root,'mobile','mobile.js')));
...
check('Compatible Rules assets and unverified Stratagem labels use the current contract',/fetch\('\.\/mobile\/related-rules\.inc\?v=\d+'\)/.test(appSource)&&/\.\/scripts\/app\.js\?v=\d+/.test(html)&&mobileRoutePages.every(page=>page.includes('mobile-route-redirect.js?v=1')&&!page.includes('mobile.js'))&&honestStratagemLabels(html)&&honestStratagemLabels(read('mobile/related-rules.inc')));
...
check('Compatible Rules renders every matrix condition',appSource.includes('conditionsFor(result)')&&read('scripts/compatible-rules-runtime.mjs').includes("'second-unit-unknown'")&&read('scripts/compatible-rules-runtime.mjs').includes("'battle-state-unknown'")&&!fs.existsSync(path.join(root,'mobile','mobile.js')));
