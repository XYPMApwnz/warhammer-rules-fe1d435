...
check('unit images remain explicitly opt-in',unitImageEntries.length===18&&unitImageEntries.every(([id,item])=>idSet.has(id)&&item.presentation.mode==='background')&&(html.match(/<figure class="unit-art-background"/g)||[]).length===18);
...
check('unit art remains decorative and layout-stable on Desktop and Phone',(html.match(/<figure class="unit-art-background" aria-hidden="true"/g)||[]).length===18&&(html.match(/<img src="\.\/assets\/unit-images\/[^"]+"[^>]+alt=""/g)||[]).length===18&&unitImagePages.every(page=>/<img src="\.\.\/assets\/unit-images\/[^"]+"[^>]+alt=""/.test(page)));
