...
check('Mechanicus Phone accepts plain and Imperium faction identities',['Adeptus Mechanicus','Imperium - Adeptus Mechanicus','Imperium \u2013 Adeptus Mechanicus','Imperium \u2014 Adeptus Mechanicus'].every(faction=>!rejectsPhone({...phoneParsed,faction})));
check('Mechanicus Phone rejects wrong faction parents',['Chaos - Adeptus Mechanicus','Xenos - Adeptus Mechanicus','Chaos \u2013 Adeptus Mechanicus','Xenos \u2014 Adeptus Mechanicus','Adeptus Mechanic',''].every(faction=>rejectsPhone({...phoneParsed,faction})));
...
const cutoutUnitImageIds=['unit-thulia-ghuld','unit-skitarii-rangers','unit-onager-dunecrawler'];
const unitImageIds=[...cutoutUnitImageIds,'unit-belisarius-cawl'];
const unitImageEntries=Object.entries(unitImageManifest.units||{});
const unitImagePages=['thulia-ghuld.html','skitarii-rangers.html','onager-dunecrawler.html','belisarius-cawl.html'].map(file=>read(`mobile/${file}`));
check('unit image proof-of-concepts remain explicitly opt-in',JSON.stringify(unitImageEntries.map(([id])=>id).sort())===JSON.stringify([...unitImageIds].sort())&&(html.match(/<figure class="unit-art"/g)||[]).length===3&&(html.match(/<figure class="unit-art-background"/g)||[]).length===1&&cutoutUnitImageIds.every(id=>unitImageManifest.units[id].presentation.mode!=='background')&&unitImageManifest.units['unit-belisarius-cawl'].presentation.mode==='background'&&read('mobile/belisarius-cawl.html').includes('<figure class="unit-art-background"'));
...
check('unit art remains decorative and layout-stable on Desktop and Phone',(html.match(/<figure class="unit-art(?:-background)?" aria-hidden="true"/g)||[]).length===4&&(html.match(/<img src="\.\/assets\/unit-images\/[^"]+"[^>]+alt=""/g)||[]).length===4&&unitImagePages.every(page=>/<img src="\.\.\/assets\/unit-images\/[^"]+"[^>]+alt=""/.test(page)));
...
check('Mega Glossary transitions use the shared return helper',/\.\.\/\.\.\/glossary-return\.js\?v=\d+/.test(html)&&popupSource.includes('WHGlossaryReturn')&&read('scripts/app.js').includes('WHGlossaryReturn'));
...
check('Compatible Rules assets and unverified Stratagem labels use the current contract',/fetch\('\.\/mobile\/related-rules\.inc\?v=\d+'\)/.test(read('scripts/app.js'))&&/fetch\('\.\/related-rules\.inc\?v=\d+'\)/.test(read('mobile/mobile.js'))&&/\.\/scripts\/app\.js\?v=\d+/.test(html)&&/\.\/mobile\.js\?v=\d+/.test(read('mobile/index.html'))&&honestStratagemLabels(html)&&honestStratagemLabels(read('mobile/related-rules.inc')));
check('desktop and Phone faction validation preserve only the canonical Imperium parent',read('scripts/roster-filter.js').includes('adeptusMechanicusFaction')&&read('mobile/mobile.js').includes('adeptusMechanicusFaction')&&!read('scripts/roster-filter.js').includes('(?:Chaos|Imperium)')&&!read('mobile/mobile.js').includes('(?:Chaos|Imperium)'));
check('Phone invalid roster handoff is terminal and non-reentrant',read('mobile/mobile.js').includes("rosterGuideHref=()=>new URL('../../../roster-guides/index.html',location.href).href")&&!read('mobile/mobile.js').includes("rosterGuideHref=()=>window.AMPhoneRoster.withRosterQuery"));
