...
const unitImagePages=unitImageIds.map(id=>read(`mobile/${id.replace('unit-','')}.html`));
check('unit images remain explicitly opt-in',unitImageEntries.length===24&&unitImageEntries.every(([id,item])=>idSet.has(id)&&item.presentation.mode==='background')&&(html.match(/<figure class="unit-art-background"/g)||[]).length===24);
check('unit image provenance is official and complete',unitImageEntries.every(([,item])=>item.unitName&&/^https:\/\/(?:www\.warhammer\.com|www\.warhammer-community\.com)\//.test(item.source.productUrl)&&/^https:\/\/(?:www\.warhammer\.com|assets\.warhammer-community\.com)\//.test(item.source.imageUrl)&&item.source.originalFilename&&item.original.endsWith(item.source.originalFilename)));
check('unit art remains decorative and layout-stable on Desktop and Phone',(html.match(/<figure class="unit-art-background" aria-hidden="true"/g)||[]).length===24&&(html.match(/<img src="\.\/assets\/unit-images\/[^"]+"[^>]+alt=""/g)||[]).length===24&&unitImagePages.every(page=>/<img src="\.\.\/assets\/unit-images\/[^"]+"[^>]+alt=""/.test(page)));
