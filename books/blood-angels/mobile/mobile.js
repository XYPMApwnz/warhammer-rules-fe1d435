...
if(rosterMode&&relatedDetachment){relatedDetachment.value='all';relatedDetachment.closest('label')?.remove();}
  ...
  const terms=Object.freeze({...window.WH40K_GLOSSARY.forBook('blood-angels')});
  window.WHGlossaryAutolink?.configure('blood-angels');
  const popups=new window.BAPhonePopups({dialog,layer:popupLayer,terms,safeFallback:()=>navButton});
  ...
  if(rosterMode){
    ...
    if(unit){window.WHBookRosterEnhancements?.decorate(unit,roster.roster,roster.entry.units);assignedEnhancementRuleIds=new Set(window.WHBookRosterEnhancements?.assignedRuleIds(roster.roster,roster.entry.units)||[]);const composition=unit.querySelector('[id$="-composition"]');if(composition){const heading=composition.querySelector('h4'),block=document.createElement('div');block.className='content-block roster-composition';const label=document.createElement('strong');label.textContent='Roster loadout';const list=document.createElement('ul');for(const item of roster.entry.units){const row=document.createElement('li');row.textContent=`${item.quantity||1}× ${item.name}${item.wargear?` — ${item.wargear}`:''}`;list.append(row);}block.append(label,list);composition.replaceChildren(heading,block);}if(roster.entry.loadout.length&&window.WHRosterEntities)unit.querySelectorAll('.weapon-row:not(.weapon-head)').forEach(row=>{const label=row.querySelector('.weapon-button')?.textContent||row.firstElementChild?.textContent;if(label&&!window.WHRosterEntities.loadoutIncludesProfile(roster.entry.loadout,label))row.remove();});}
  ...
  async function loadRelated(){
    ...
    try{
      const [response,matrix]=await Promise.all([fetch('./related-rules.inc?v=1'),compatibleRuntime.loadCompatibleRules(new URL('../generated/compatible-rules.json',scriptUrl))]);if(!response.ok)throw new Error(`HTTP ${response.status}`);
  ...
  if(relatedRulesEnabled&&relatedDetachment&&!rosterMode){try{const saved=localStorage.getItem('blood-angels-detachment-filter');if(saved&&relatedDetachment.querySelector(`option[value="${CSS.escape(saved)}"]`))relatedDetachment.value=saved;}catch{}relatedDetachment.addEventListener('change',()=>{try{localStorage.setItem('blood-angels-detachment-filter',relatedDetachment.value);}catch{}filterRelated();});}
