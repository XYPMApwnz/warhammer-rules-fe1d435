(function(){
  ...
  if(faction(roster?.faction)!=='blood angels'||!roster?.units?.length){location.replace('../../roster-guides/index.html');return;}
  ...
  window.BA_ROSTER_GUIDE=Object.freeze({detachmentIds,enhancementRuleIdsByUnitId});
  document.title="Blood Angels Roster Guide";document.querySelector('.app-brand strong').textContent="Blood Angels Roster Guide";document.querySelector('[data-roster-guides]')?.removeAttribute('hidden');
  const hero=document.querySelector('#start');if(hero){hero.querySelector('.eyebrow').textContent='Personal roster guide';hero.querySelector('h1').textContent=record.name||"Blood Angels";hero.querySelector('h1 + p').textContent=`${roster.units.length} units · ${labels.join(' + ')}`;}
  ...
  document.querySelectorAll('.unit-card').forEach(card=>{const entry=selected.get(card.id);if(!entry){card.remove();return;}card.dataset.rosterSelected='true';enhancementRuleIdsByUnitId[card.id]=window.WHBookRosterEnhancements?.assignedRuleIds(roster,entry.units)||[];window.WHBookRosterEnhancements?.decorate(card,roster,entry.units);const status=card.querySelector('.unit-status');if(status)status.textContent=`${entry.units.length>1?`${entry.units.length} units · `:''}${entry.points} pts`;if(entry.loadout.length&&window.WHRosterEntities)card.querySelectorAll('.weapon-row:not(.weapon-head)').forEach(row=>{const label=row.querySelector('.weapon-button')?.textContent||row.firstElementChild?.textContent;if(label&&!window.WHRosterEntities.loadoutIncludesProfile(entry.loadout,label))row.remove();});card.querySelectorAll('.weapon-group').forEach(group=>{if(!group.querySelector('.weapon-row:not(.weapon-head)'))group.remove();});});
