...
const enhancements=Object.values(context.window.WH_BOOK_ROSTER_ENHANCEMENTS),local=enhancements.filter(item=>!item.sourceId),provider=fs.readFileSync(new URL('../../extensions/book-roster-enhancement-providers.js',import.meta.url),'utf8'),contracts=JSON.parse(fs.readFileSync(new URL('../sources/blood-angels-effect-contracts.v1.json',import.meta.url),'utf8')).contracts,localEffectContracts=contracts.filter(item=>item.sourceKind==='enhancement');
...
for(const item of local)assert.equal(localEffectContracts.filter(contract=>contract.canonicalRecordId===item.ruleId).length,1,`${item.ruleId}: one source-owned effect contract`);
