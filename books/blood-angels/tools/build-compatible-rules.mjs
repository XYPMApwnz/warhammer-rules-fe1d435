...
function target(contract){
  const roles=contract?.roles||contract?.targets||[],role=roles.find(item=>item.side==='friendly'&&item.subject==='unit');
...
export function buildCompatibleRules({config,pack,parity,codex,points,contracts,spaceMarines}){
  ...
  const detachments=[...(pack.detachments||[]),...(parity.detachments||[])];
  if(detachments.length!==8||new Set(detachments.map(item=>item.id)).size!==8)throw new Error(`Expected 8 unique Detachments, got ${detachments.length}`);
