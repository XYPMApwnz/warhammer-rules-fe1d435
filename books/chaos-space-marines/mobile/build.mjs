...
const categories=[...source.matchAll(/<section class="content-group" id="(datasheets-[^"]+)"[^>]*>\s*<h3 class="category-title">([^<]+)<\/h3>/g)].map(([,id,title])=>{
  ...
  const units=[...section.matchAll(/<article\b[^>]*\bclass="[^"]*\bunit-card\b[^"]*"[^>]*\bid="(unit-[^"]+)"[^>]*>/g)].map(([,unitId])=>{
    ...
    return{id:unitId,title:clean(unitTitle),file:`${unitId.slice(5)}.html`};
  });
...
const routes=[{file:'index.html',id:'start',title:'Start'},{file:'army-rules.html',id:'army-rules',title:'Army Rules'},{file:'updates.html',id:'updates',title:'Updates'},...detachments,...units];
...
const page=route=>`<!doctype html><html lang="en" data-canonical-reader="../reader.html" data-canonical-target="${attribute(route.id)}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${attribute(route.title)} — Chaos Space Marines</title><script src="../../shared/mobile-route-redirect.js?v=1"></script></head><body><p>Opening the canonical Chaos Space Marines reader…</p></body></html>\n`;
...
if(check){
  ...
  if(errors.length){console.error(errors.join('\n'));process.exitCode=1;}else console.log(`Chaos Space Marines compatibility build check: ${routes.length} content-free routes are fresh.`);
}else{
  ...
  console.log(`Chaos Space Marines compatibility routes built: ${routes.length} stubs, ${detachments.length} detachments and ${units.length} datasheets.`);
}
