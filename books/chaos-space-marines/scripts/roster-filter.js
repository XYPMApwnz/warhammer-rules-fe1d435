(function(){
  ...
  const normalize=value=>String(value||'').toLowerCase().replace(/\s*\[legends\]\s*$/i,'').replace(/\s*\(aura\)\s*$/i,'').replace(/[^a-z0-9]+/g,' ').trim(),faction=value=>normalize(String(value||'').replace(/^Chaos\s*[-–—]\s*/i,'')),slug=value=>normalize(value).replace(/\s+/g,'-');
  if(faction(roster?.faction)!=='chaos space marines'||!roster?.units?.length){location.replace('../../roster-guides/index.html');return;}
  ...
  for(const [cardId,entry] of selected)enhancementRuleIdsByUnitId[cardId]=window.WHBookRosterEnhancements?.assignedRuleIds(roster,entry.units)||[];
  window.CSM_ROSTER_GUIDE=Object.freeze({detachmentIds,enhancementRuleIdsByUnitId:Object.freeze(enhancementRuleIdsByUnitId)});
  document.title="Chaos Space Marines Roster Guide";document.querySelector('.app-brand strong').textContent="Chaos Space Marines Roster Guide";document.querySelector('[data-roster-guides]')?.removeAttribute('hidden');
  const hero=document.querySelector('#start');if(hero){hero.querySelector('.eyebrow').textContent='Personal roster guide';hero.querySelector('h1').textContent=record.name||"Chaos Space Marines";hero.querySelector('h1 + p').textContent=`${roster.units.length} units · ${labels.join(' + ')}`;}
  ...
  document.querySelectorAll('.unit-card').forEach(card=>{const entry=selected.get(card.id);if(!entry){card.remove();return;}card.dataset.rosterSelected='true';const status=card.querySelector('.unit-status');if(status)status.textContent=`${entry.units.length>1?`${entry.units.length} units · `:''}${entry.points} pts`;window.WHBookRosterEnhancements?.decorate(card,roster,entry.units);if(entry.loadout.length&&window.WHRosterEntities)card.querySelectorAll('.weapon-row:not(.weapon-head)').forEach(row=>{const label=row.querySelector('.weapon-button')?.textContent||row.firstElementChild?.textContent;if(label&&!window.WHRosterEntities.loadoutIncludesProfile(entry.loadout,label))row.remove();});card.querySelectorAll('.weapon-group').forEach(group=>{if(!group.querySelector('.weapon-row:not(.weapon-head)'))group.remove();});});
