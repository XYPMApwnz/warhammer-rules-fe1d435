...
const glossaryContext=JSON.parse(await readFile(new URL('../../../glossary/contexts/dark-angels.json',import.meta.url),'utf8')).terms;
...
if(detachments.length!==24||localUnits.length!==16||sharedUnits.length!==82)throw new Error(`Expected 24 Detachments, 16 Dark Angels and 82 Space Marines Datasheets; found ${detachments.length}, ${localUnits.length} and ${sharedUnits.length}`);
...
for(const route of routes)for(const[,id]of rawContent(route).matchAll(/\sid="([^"]+)"/g))mobileRulePaths.set(id,`books/dark-angels/mobile/${route.file}#${id}`);
...
for(const context of Object.values(glossaryContext)){
  ...
  if(/^books\/dark-angels\/(?:index|reader)\.html#/.test(path)&&!mobileRulePaths.has(anchor))throw new Error(`Missing mobile rule route for ${path}`);
}
...
function page(route){
  ...
  return `<!doctype html><html lang="en" data-theme="dark" data-book-id="dark-angels"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#0a0b0d"><title>${route.title} &mdash; Dark Angels</title><link rel="manifest" href="../../../manifest.webmanifest"><link rel="stylesheet" href="../../death-guard/styles/tokens.css?v=11"><link rel="stylesheet" href="../styles/tokens.css?v=2"><link rel="stylesheet" href="../../death-guard/styles/layout.css?v=11"><link rel="stylesheet" href="../../death-guard/styles/navigation.css?v=12"><link rel="stylesheet" href="../../death-guard/styles/content.css?v=41"><link rel="stylesheet" href="../../death-guard/styles/popups.css?v=18"><link rel="stylesheet" href="../../shared/datasheet-system.css?v=8"><link rel="stylesheet" href="../styles/book.css?v=3"><link rel="stylesheet" href="./mobile.css?v=1"></head><body><header class="app-header" id="appHeader"><button class="header-button nav-menu" id="navButton" aria-label="Open navigation" aria-controls="mobileNav" aria-expanded="false">&#9776;</button><div class="app-brand"><strong>Dark Angels Rules</strong><small>11E &middot; Source-limited mobile preview</small></div><a class="library-link" href="../../../index.html" aria-label="Back to rulebook library"><span aria-hidden="true">&larr;</span><b>Library</b></a><div class="header-spacer"></div></header><button class="toc-scrim" id="navScrim" aria-label="Close navigation" hidden></button><nav class="toc-panel" id="mobileNav" aria-label="Dark Angels navigation" aria-hidden="true"><h2 class="toc-heading">Contents</h2><div class="phone-shortcuts"><a class="phone-glossary" href="../../../glossary/index.html">Mega Glossary &rarr;</a><a class="phone-glossary phone-mode-switch" href="../reader.html#${route.id}" data-view-switch>Desktop / iPad view &rarr;</a></div><div class="phone-tree">${navigation(route)}</div></nav><main class="main mobile-main"><article class="document">${hydrateTerms(rawContent(route))}${related}</article><a class="return-popup" id="returnPopup" hidden>Return to popup</a></main><script src="../../shared/datasheet-layout.js?v=3"></script><script src="../../shared/rule-facts.js?v=4"></script><script src="../../shared/related-rules-matcher.js?v=6"></script><script src="../../shared/army-related-rules.js?v=10"></script><dialog class="mobile-dialog" id="termDialog" aria-label="Term reference"><div class="mobile-popup-stack" id="termPopupStack"></div></dialog><script src="../../../glossary/generated/glossary.en.js?v=dark-angels-1"></script><script src="../scripts/data.js?v=2"></script><script src="../scripts/roster-data.js?v=1"></script><script src="../../shared/book-roster-enhancements.js?v=1"></script><script src="../../../glossary-return.js?v=3"></script><script src="../../shared/popup-rule-actions.js?v=1"></script><script src="../../shared/popup-content.js?v=3"></script><script src="../../shared/glossary-autolink.js?v=8"></script><script src="./phone-popup-controller.js?v=1"></script><script src="./mobile.js?v=2"></script></body></html>\n`;
}
...
if(check){
  ...
  if(errors.length){console.error(errors.join('\n'));process.exitCode=1;}else console.log(`Dark Angels Phone build check: ${routes.length} routes are fresh.`);
}else{
  ...
  console.log(`Dark Angels mobile reference built: ${routes.length} pages, ${detachments.length} detachments, ${localUnits.length} local and ${sharedUnits.length} shared datasheets.`);
}
