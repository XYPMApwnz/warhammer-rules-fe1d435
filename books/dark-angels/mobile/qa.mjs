...
for(const route of routes){const html=fs.readFileSync(path.join(root,route),'utf8');assert.match(html,/\.\/mobile\.js\?v=2/);assert.match(html,/\.\/phone-popup-controller\.js\?v=1/);assert.doesNotMatch(html,/reader\.html\?view=mobile/);assert.match(html,/data-view-switch/);}
...
assert.match(fs.readFileSync(path.join(root,'../reader.html'),'utf8'),/\.\/scripts\/app\.js\?v=5/,'Dark Angels Desktop app version must include Compatible Rules');
assert.match(desktopApp,/WHRelatedRules.*enabled:true/,'Dark Angels Desktop must enable the existing Compatible Rules matcher');
...
const terminators=fs.readFileSync(path.join(root,'terminator-squad.html'),'utf8'),mobileRuntime=fs.readFileSync(path.join(root,'mobile.js'),'utf8');assert.equal(config.generatedMobile,true);assert.equal(manifest.gates.publishAsComplete,false);for(const html of [belial,terminators]){assert.match(html,/id="relatedRules"/);assert.match(html,/Compatible Stratagems &amp; Enhancements/);assert.match(html,/\.\.\/scripts\/data\.js\?v=2/);}assert.match(mobileRuntime,/faction\(roster\?\.faction\)!=='dark angels'/);assert.match(mobileRuntime,/link\.remove\(\)/);assert.match(mobileRuntime,/destination\.searchParams\.set\('roster'/);assert.match(mobileRuntime,/WHArmyRelatedRules\.match\(card,profile\)/);assert.match(mobileRuntime,/new Set\(roster\.detachments\)/);assert.match(mobileRuntime,/window\.DG_TERMS/);
