(async function () {
  ...
  if (rosterMode) {
    try {
      ...
      const stateKey=unit=>[unit.points||0,unit.warlord?1:0,unitRows(unit).map(row=>`${row[0]||0}:${normalize(row[1])}:${normalize(row[2])}`).sort().join('|'),unitLoadout(unit).map(normalize).sort().join('|'),(enhancementsByOwnerId.get(unit.id)||[]).map(item=>`${item.ruleId||item.id||slug(item.name)}:${item.points||0}:${slug(item.detachment)}`).sort().join('|'),attachmentStateKey(unit)].join('\0');
