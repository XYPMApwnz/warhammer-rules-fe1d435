(async function () {
  ...
  const renderAttachedUnits=(card,units,roster,attachments)=>{
    ...
    units.forEach(instance=>{const group=groups.find(item=>item.ids.has(instance.id)),biologus=group?.characters.find(item=>normalize(item.name)==='biologus putrifier')||null,tallyman=group?.characters.find(item=>normalize(item.name)==='tallyman')||null,eyeOfAffliction=eyeOwners.some(item=>item.ownerUnitId===instance.id||group?.ids.has(item.ownerUnitId)),revoltingRegeneration=revoltingOwners.some(item=>item.ownerUnitId===instance.id),isBodyguard=group?.bodyguard.id===instance.id,vectorSource=group?.characters.find(item=>hasCanonicalRule(item,'ability-vector-of-disease-2498580'))||null,giftSource=group?.characters.find(item=>hasCanonicalRule(item,'ability-gift-of-contagion-psychic-4fea300'))||null,blindingSource=group?.characters.find(item=>hasCanonicalRule(item,'ability-blinding-spray-7d189f5'))||null,silentBodyguard=Boolean(group&&!isBodyguard&&hasCanonicalRule(group.bodyguard,'ability-silent-bodyguard-03a0a1b')&&canonicalUnitKeywords(instance).includes('CHARACTER')),archContaminator=Boolean(isBodyguard&&archOwners.some(item=>group?.ids.has(item.ownerUnitId)));if(!group&&!eyeOfAffliction&&!revoltingRegeneration)return;const article=document.createElement('article');article.className='ability roster-attached-unit';article.dataset.rosterInstanceId=instance.id;if(group)article.dataset.aggregateUnitKeywords=group.aggregateUnitKeywords.join('|');const heading=document.createElement('h5');heading.textContent=`${labels.get(instance.id)} · ${group?'Attached Unit':'Enhancement projection'}`;article.append(heading);if(group){const members=document.createElement('p');members.textContent=`Attached Unit: ${[group.bodyguard,...group.characters].map(item=>labels.get(item.id)||item.name).join(', ')}`;article.append(members);}const uniqueWeaponProjection=units.length===1;if(biologus){const note=document.createElement('p');note.className='roster-derived-note roster-foul-infusion';note.textContent='Foul Infusion: these weapon profiles have LETHAL HITS; unmodified Hit rolls of 5+ score a Critical Hit.';article.append(note);if(uniqueWeaponProjection)card.querySelectorAll('.weapon-row:not(.weapon-head)').forEach(addLethalHits);}if(tallyman){const note=document.createElement('p');note.className='roster-derived-note roster-malicious-calculations';note.textContent='Malicious Calculations: when a model in this Attached Unit attacks, you can ignore modifiers to its BS or WS and/or modifiers to the Hit roll.';article.append(note);}if(vectorSource){if(uniqueWeaponProjection){[...card.querySelectorAll('.weapon-group')].filter(weaponGroup=>normalize(weaponGroup.querySelector('h5')?.textContent).includes('melee weapons')).forEach(weaponGroup=>weaponGroup.querySelectorAll('.weapon-row:not(.weapon-head)').forEach(row=>{addWeaponAbility(row,'SUSTAINED HITS 1','core-sustained-hits');addWeaponAbility(row,'LANCE','core-lance');}));}else{const note=document.createElement('p');note.className='roster-derived-note roster-vector-of-disease';note.textContent=`Vector of Disease: ${labels.get(instance.id)} melee weapons have SUSTAINED HITS 1 and LANCE.`;article.append(note);}}if(silentBodyguard){const note=document.createElement('p');note.className='roster-derived-note roster-silent-bodyguard';note.textContent='Silent Bodyguard: this CHARACTER model has Feel No Pain 4+ while leading Deathshroud Terminators.';article.append(note);}if(isBodyguard&&giftSource){const note=document.createElement('p');note.className='roster-derived-note roster-gift-of-contagion';note.textContent='Gift of Contagion: while this Attached Unit targets an AFFLICTED unit, those attacks have SUSTAINED HITS 1.';article.append(note);}if(archContaminator){const note=document.createElement('p');note.className='roster-derived-note roster-arch-contaminator';note.textContent='Arch Contaminator: while this Attached Unit is within range of an objective marker you control, models in it can re-roll the Wound roll.';article.append(note);}if(isBodyguard&&blindingSource){const note=document.createElement('p');note.className='roster-derived-note roster-blinding-spray';note.textContent='Blinding Spray (activation): in the Fight phase, Foul Blightspawn can give this Attached Unit Fights First until the end of the phase; once per battle.';article.append(note);}if(eyeOfAffliction){const note=document.createElement('p');note.className='roster-derived-note roster-eye-of-affliction-note';note.textContent='Eye of Affliction: ranged weapons have IGNORES COVER while targeting an AFFLICTED enemy unit.';article.append(note);if(uniqueWeaponProjection)[...card.querySelectorAll('.weapon-group')].filter(weaponGroup=>normalize(weaponGroup.querySelector('h5')?.textContent).includes('ranged weapons')).forEach(weaponGroup=>weaponGroup.querySelectorAll('.weapon-row:not(.weapon-head)').forEach(addEyeOfAffliction));}if(revoltingRegeneration){const note=document.createElement('p');note.className='roster-derived-note roster-revolting-regeneration';note.textContent='Revolting Regeneration: this bearer has Feel No Pain 5+.';article.append(note);}root.append(article);});
  };
