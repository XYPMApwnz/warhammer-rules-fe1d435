(async function () {
  ...
  const {slug,normalize,unitLoadout,unitRows,unitModelCount}=semantics;
  ...
  if (rosterMode) {
    try {
      ...
      if(matching.length){
        ...
        semantic.decorate(unit,matching,rosterDetachmentIds);
        if(active.loadout.length){unit.querySelectorAll('.weapon-row:not(.weapon-head)').forEach(row=>{const weapon=row.querySelector('.weapon-button')?.textContent||row.firstElementChild?.textContent;if(weapon&&!window.WHRosterEntities.loadoutIncludesProfile(active.loadout,weapon))row.remove();});unit.querySelectorAll('.weapon-group').forEach(group=>{if(!group.querySelector('.weapon-row:not(.weapon-head)'))group.remove();});}
