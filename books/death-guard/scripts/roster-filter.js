(function () {
  ...
  const renderAttachedUnits=(card,units)=>{
    ...
    units.forEach(instance=>{
      ...
      const uniqueWeaponProjection=units.length===1;
      if(biologus){
        ...
        if(uniqueWeaponProjection)card.querySelectorAll('.weapon-row:not(.weapon-head)').forEach(addLethalHits);
      }
      ...
      if(vectorSource){if(uniqueWeaponProjection){[...card.querySelectorAll('.weapon-group')].filter(weaponGroup=>normalize(weaponGroup.querySelector('h5')?.textContent).includes('melee weapons')).forEach(weaponGroup=>weaponGroup.querySelectorAll('.weapon-row:not(.weapon-head)').forEach(row=>{addWeaponAbility(row,'SUSTAINED HITS 1','core-sustained-hits');addWeaponAbility(row,'LANCE','core-lance');}));}else{const note=document.createElement('p');note.className='roster-derived-note roster-vector-of-disease';note.textContent=`Vector of Disease: ${rosterInstanceLabels.get(instance.id)} melee weapons have SUSTAINED HITS 1 and LANCE.`;article.append(note);}}
      ...
      if(eyeOfAffliction){const note=document.createElement('p');note.className='roster-derived-note roster-eye-of-affliction-note';note.textContent='Eye of Affliction: ranged weapons have IGNORES COVER while targeting an AFFLICTED enemy unit.';article.append(note);if(uniqueWeaponProjection)[...card.querySelectorAll('.weapon-group')].filter(weaponGroup=>normalize(weaponGroup.querySelector('h5')?.textContent).includes('ranged weapons')).forEach(weaponGroup=>weaponGroup.querySelectorAll('.weapon-row:not(.weapon-head)').forEach(addEyeOfAffliction));}
