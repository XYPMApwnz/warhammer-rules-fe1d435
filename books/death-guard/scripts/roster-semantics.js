(function (root) {
  ...
  const createContext = ({roster, attachments = {}, terms = {}, profileFor = () => null}) => {
    ...
    const projectEffects = (unit, cardId, detachmentIds = [], gameUnit = null) => {
      ...
      const pipes=attachedOwner(DG_ENH.pipes);if(canonicalUnitId(bodyguard)==='unit-poxwalkers'&&pipes){stat('witherbone-pipes-oc','OC','add',1,attachmentSource(DG_ENH.pipes,pipes));if(pipes.id!==unit.id)canonicalEnhancement(DG_ENH.pipes,attachmentSource(DG_ENH.pipes,pipes));}
