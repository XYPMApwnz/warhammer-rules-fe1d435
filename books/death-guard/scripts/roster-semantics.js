(function (root) {
  ...
  const DG_RULE = {
    ...
    virulent:'ability-virulent-aura-c28aa51', vitality:'ability-sickening-vitality-89bb5ff', malicious:'ability-malicious-calculations-8505f03', blinding:'ability-blinding-spray-7d189f5', tainted:'ability-tainted-narthecium-01ba1bd',
  ...
  const DG_ENH = {
    ...
    talisman:'enhancement-talisman-of-burgeoning', vigour:'enhancement-vile-vigour', helm:'enhancement-helm-of-the-fly-king', needle:'enhancement-needle-of-nurgle', plagueveil:'enhancement-plagueveil',
    bilemaw:'enhancement-bilemaw-blight', daemonWeapon:'enhancement-daemon-weapon-of-nurgle', archContaminator:'enhancement-arch-contaminator',
    eyeOfAffliction:'enhancement-eye-of-affliction', shriekworm:'enhancement-shriekworm-familiar', finalIngredient:'enhancement-final-ingredient',
    walkingPox:'enhancement-lord-of-the-walking-pox', warprot:'enhancement-warprot-talisman', rejuvenating:'enhancement-rejuvenating-swarm', hybridisedPox:'enhancement-host-of-the-hybridised-pox'
  };
  ...
  const createContext = ({roster, attachments = {}, terms = {}, profileFor = () => null}) => {
    ...
    const projectEffects = (unit, cardId, detachmentIds = [], gameUnit = null) => {
      ...
      const canonicalReference = (kind, id, origin) => add(`canonical-${id}`,'ability',id,'reference',{canonicalReference:{kind,id},...(kind==='ability'?{canonicalAbilityId:id}:{}),state:'reference'},origin);
      const canonicalAbility = (id, origin) => canonicalReference('ability',id,origin);
      const canonicalEnhancement = (id, origin) => canonicalReference('enhancement',id,origin);
      const canonicalDetachmentRule = (id, origin) => canonicalReference('detachment-rule',id,origin);
      ...
      if (detachments.has('detachment-paragons-of-putrescence') && (profileByRosterId.get(unit.id)?.keywords||[]).some(item=>normalize(item?.title||item) === 'character')) canonicalDetachmentRule(DG_RULE.strains,detachmentSource('paragons-of-putrescence'));
      ...
      const virulence=leader('unit-lord-of-virulence');if(virulence&&cardId!=='unit-lord-of-virulence')canonicalAbility(DG_RULE.virulent,attachmentSource(DG_RULE.virulent,virulence));
      ...
      const pipes=attachedOwner(DG_ENH.pipes);if(canonicalUnitId(bodyguard)==='unit-poxwalkers'&&pipes){stat('witherbone-pipes-oc','OC','add',1,attachmentSource(DG_ENH.pipes,pipes));if(pipes.id!==unit.id)canonicalEnhancement(DG_ENH.pipes,attachmentSource(DG_ENH.pipes,pipes));}const sorrowsyphon=attachedOwner(DG_ENH.sorrowsyphon);if(canonicalUnitId(bodyguard)==='unit-poxwalkers'&&sorrowsyphon&&sorrowsyphon.id!==unit.id)canonicalEnhancement(DG_ENH.sorrowsyphon,attachmentSource(DG_ENH.sorrowsyphon,sorrowsyphon));
      ...
      const vigour=attachedOwner(DG_ENH.vigour);if(vigour){stat('vile-vigour-move','M','add',1,attachmentSource(DG_ENH.vigour,vigour));if(vigour.id!==unit.id)canonicalEnhancement(DG_ENH.vigour,attachmentSource(DG_ENH.vigour,vigour));}
      const helm=attachedOwner(DG_ENH.helm);if(helm&&helm.id!==unit.id)canonicalEnhancement(DG_ENH.helm,attachmentSource(DG_ENH.helm,helm));
      const needle=attachedOwner(DG_ENH.needle);if(needle&&unit.id===bodyguard?.id)canonicalEnhancement(DG_ENH.needle,attachmentSource(DG_ENH.needle,needle));
      for(const id of [DG_ENH.archContaminator,DG_ENH.eyeOfAffliction,DG_ENH.shriekworm,DG_ENH.finalIngredient,DG_ENH.warprot,DG_ENH.rejuvenating,DG_ENH.hybridisedPox]){const owner=attachedOwner(id);if(owner&&owner.id!==unit.id)canonicalEnhancement(id,attachmentSource(id,owner));}
      const walkingPox=attachedOwner(DG_ENH.walkingPox);if(canonicalUnitId(bodyguard)==='unit-poxwalkers'&&walkingPox&&walkingPox.id!==unit.id)canonicalEnhancement(DG_ENH.walkingPox,attachmentSource(DG_ENH.walkingPox,walkingPox));
      return output;
    };
    ...
    const decorate = (card, units, detachmentIds = [], projectedEffects = null, applyProjectedEffects = true) => {
      ...
      if (Array.isArray(projectedEffects)) {
        ...
        const sharedEnhancementEffects=new Set(['furnace-attacks','furnace-strength','furnace-devastating','critical-hit-5','melee-a-2','plague-wind-range','mobile-keyword']);
      ...
      if (entryEvery((unit) => Boolean(attachedEnhancementOwner(unit, DG_ENH.vigour)))) modifyModelStat('M', 1, 'vile-vigour');
