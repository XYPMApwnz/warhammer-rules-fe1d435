(async function(){
  ...
  if(relatedRulesEnabled&&relatedDetachment){try{const saved=localStorage.getItem('emperors-children-detachment-filter');if(saved&&relatedDetachment.querySelector(`option[value="${CSS.escape(saved)}"]`))relatedDetachment.value=saved;}catch{}relatedDetachment.addEventListener('change',()=>{try{localStorage.setItem('emperors-children-detachment-filter',relatedDetachment.value);}catch{}filterRelated();});}
