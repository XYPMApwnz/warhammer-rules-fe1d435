(async function(){
  ...
  const normalize=value=>String(value||'').toLowerCase().replace(/\s*\[legends\]\s*$/i,'').replace(/\s*\(aura\)\s*$/i,'').replace(/[^a-z0-9]+/g,' ').trim(),slug=value=>normalize(value).replace(/\s+/g,'-'),normalizeEmperorsChildrenFaction=value=>normalize(String(value||'').replace(/^Chaos\s*[-–—]\s{0,}/i,''));
  function rosterContext(){
    ...
    if(!['emperor s children','emperors children'].includes(normalizeEmperorsChildrenFaction(roster?.faction))||!roster?.units?.length)return null;
...
if(rosterMode&&relatedDetachment){relatedDetachment.value='all';relatedDetachment.closest('label')?.remove();}
  ...
  const popups=new window.ECPhonePopups({dialog,layer:popupLayer,terms,safeFallback:()=>navButton});
  ...
  if(relatedRulesEnabled&&relatedDetachment&&!rosterMode){try{const saved=localStorage.getItem('emperors-children-detachment-filter');if(saved&&relatedDetachment.querySelector(`option[value="${CSS.escape(saved)}"]`))relatedDetachment.value=saved;}catch{}relatedDetachment.addEventListener('change',()=>{try{localStorage.setItem('emperors-children-detachment-filter',relatedDetachment.value);}catch{}filterRelated();});}
