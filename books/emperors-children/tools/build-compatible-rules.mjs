...
export function inputs(){
  ...
  return {datasheets:codex.datasheets||[],pack:read('content/emperors-children-faction-pack.en.json'),snapshot:read('sources/wahapedia-compatible-rules.snapshot.json'),ledger:read('sources/compatible-rules-correction-ledger.json'),owners:read('sources/enhancement-owner-matrix.json')};
}
export function buildCompatibleRules({datasheets,pack,snapshot,ledger,owners}){
  const detachmentByRule=new Map(pack.detachments.flatMap(detachment=>detachment.stratagems.map(rule=>[rule.id,detachment.id]))),rows=new Map(datasheets.map(unit=>[unit.id,new Map()]));
  ...
  for(const [unitId,ruleIds] of Object.entries(snapshot.units))for(const ruleId of ruleIds)add(unitId,ruleId,{detachmentId:detachmentByRule.get(ruleId),state:'match',authority:'official-faction-pack-v1.0'});
  ...
  return {schema:'emperors-children-compatible-rules/v1',scope:{datasheets:datasheets.length,officialFactionStratagems:15,coreStratagems:10,excludedCodexCarryForwardStratagems:36},units:Object.fromEntries([...rows].sort(([a],[b])=>a.localeCompare(b,'en')).map(([unitId,rules])=>[unitId,[...rules.values()].sort((a,b)=>a.ruleId.localeCompare(b.ruleId,'en'))]))};
}
