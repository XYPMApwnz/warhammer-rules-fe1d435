(function(root){
  ...
  function effectLabel(effect){
    if(effect.component==='ability'){const title=effect.title||'Granted ability';return [effect.ruleTitle,effect.operation==='remove'?`${title} removed`:title].filter(Boolean).join(' · ');}
  ...
  function applyEffectiveAbilities(card,gameUnit,projection){
    ...
    for(const effect of list(gameUnit.effects).filter(effect=>effect.component==='ability')){
      const reference=effect.operation==='reference'?(effect.canonicalReference||effect.canonicalAbility):null,title=reference?.title||effect.title||effect.targetId,identity=reference?.id||effect.targetId||title,candidates=[...card.querySelectorAll('.ability,.roster-enhancement,[data-ability-id],[data-roster-enhancement],[data-term]')],matches=candidates.filter(node=>[node.dataset.rosterCanonicalReferenceId,node.dataset.rosterCanonicalAbilityId,node.dataset.rosterCanonicalEnhancementId,node.dataset.rosterRuleId,node.dataset.abilityId,node.dataset.rosterEnhancement,node.dataset.term,node.id].some(value=>referenceTitle(value)===referenceTitle(identity))||referenceTitle(node.querySelector('h5')?.textContent)===referenceTitle(title)||referenceTitle(node.textContent)===referenceTitle(title)),existing=matches[0];
