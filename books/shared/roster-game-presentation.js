(function(root){
  ...
  function applyEffectiveAbilities(card,gameUnit,projection){const section=partEnding(card,'-abilities'),listNode=section?.querySelector('.ability-list');if(!listNode)return;for(const effect of list(gameUnit.effects).filter(effect=>effect.component==='ability')){const reference=effect.operation==='reference'?effect.canonicalAbility:null,title=reference?.title||effect.title||effect.targetId,identity=reference?.id||effect.targetId||title,existing=[...listNode.querySelectorAll(':scope > .ability')].find(node=>[node.dataset.rosterCanonicalAbilityId,node.dataset.rosterRuleId,node.id,node.querySelector('h5')?.textContent].some(value=>String(value||'').trim().toLowerCase()===String(identity||'').toLowerCase())||String(node.querySelector('h5')?.textContent||'').trim().toLowerCase()===String(title||'').trim().toLowerCase());if(effect.operation==='remove'&&activeEffect(effect)){if(existing)existing.hidden=true;continue;}if(activeEffect(effect)&&effect.operation==='grant'&&String(effect.targetId||'').startsWith('core-')){const core=[...listNode.querySelectorAll(':scope > .ability')].find(node=>String(node.querySelector('h5')?.textContent||'').trim().toLowerCase()==='core'),body=core?.querySelector('p');if(body){const normalized=String(title||'').trim().toLowerCase(),present=body.textContent.split(',').some(value=>value.trim().toLowerCase()===normalized)||[...body.querySelectorAll('[data-roster-effect-id]')].some(node=>node.dataset.rosterEffectId===effect.id);if(!present){if(body.textContent.trim())body.append(document.createTextNode(', '));const granted=element('span','',title);granted.dataset.rosterEffectId=effect.id;body.append(granted);}continue;}}if(!title)continue;if(reference&&existing){existing.dataset.rosterCanonicalAbilityId=reference.id;existing.dataset.rosterCanonicalSectionId=reference.sectionId||'';const source=sourceTitle(effect,projection),heading=existing.querySelector('h5');if(source&&heading&&!existing.querySelector('.roster-game-ability-source'))heading.after(element('div','roster-game-ability-source',source));continue;}if(existing)continue;const article=element('article',`ability roster-game-derived-ability${reference?' roster-game-canonical-reference':activeEffect(effect)?'':' roster-game-conditional'}`),heading=element('h5','',title);article.dataset.rosterEffectId=effect.id;if(reference){article.dataset.rosterCanonicalAbilityId=reference.id;article.dataset.rosterCanonicalSectionId=reference.sectionId||'';const source=sourceTitle(effect,projection);article.append(heading);if(source)article.append(element('div','roster-game-ability-source',source));article.append(element('p','',reference.text||''));}else article.append(heading,element('p','',effect.summary||'Roster effect reference.'));listNode.prepend(article);}}
