(function(root){
  ...
  function effectLabel(effect){
    if(effect.component==='ability')return [effect.ruleTitle,effect.title||'Granted ability'].filter(Boolean).join(' · ');
  ...
  function applyEffectiveAbilities(card,gameUnit,projection){
    ...
    for(const effect of list(gameUnit.effects).filter(effect=>effect.component==='ability')){
      const reference=effect.operation==='reference'?(effect.canonicalReference||effect.canonicalAbility):null,title=reference?.title||effect.title||effect.targetId,identity=reference?.id||effect.targetId||title,existing=[...card.querySelectorAll('.ability,.roster-enhancement,[data-ability-id],[data-roster-enhancement]')].find(node=>[node.dataset.rosterCanonicalReferenceId,node.dataset.rosterCanonicalAbilityId,node.dataset.rosterCanonicalEnhancementId,node.dataset.rosterRuleId,node.dataset.abilityId,node.dataset.rosterEnhancement,node.id].some(value=>referenceTitle(value)===referenceTitle(identity))||referenceTitle(node.querySelector('h5')?.textContent)===referenceTitle(title));
      if(effect.operation==='remove'&&activeEffect(effect)){if(existing)existing.hidden=true;continue;}
