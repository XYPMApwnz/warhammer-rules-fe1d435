(function(root){
  ...
  function effectLabel(effect){
    ...
    if(effect.component==='stat')return `${effect.targetId} ${effect.operation==='set'?`set to ${effect.to}`:signed(effect.delta)}`;
  ...
  function presentEffects(card,gameUnit,projection){
    ...
    const rows=element('ul','');for(const effect of effects){const label=effectLabel(effect);if(!label)continue;const item=element('li','');item.append(element('span','',label));const source=sourceTitle(effect,projection);if(source)item.append(element('small','',source));rows.append(item);}
  ...
  function present(card,gameUnit,projection){
    ...
    replaceCost(card,gameUnit);buildSummary(card,gameUnit,projection);filterWeapons(card,gameUnit);filterWargearAbilities(card,gameUnit);transformComposition(card,gameUnit);omitBuildGuidance(card,gameUnit);presentEffects(card,gameUnit,projection);emphasizeKeywords(card,gameUnit);syncNavigation(card);return card;
  }
