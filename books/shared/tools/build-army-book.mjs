...
const enhancementRuleId=item=>config.id==='chaos-space-marines'&&item.id?item.id:config.id==='space-marines'&&(item.sourceId||item.id)?item.sourceId||item.id:enhancementOwnerRecords.find(record=>titleKey(record.title)===titleKey(item.title))?.id||item.id||`enhancement-${slug(item.title)}`;
...
const rosterEnhancements=Object.fromEntries(detachments.flatMap(det=>(det.enhancements||[]).filter(item=>!enhancementOwnerRecord(item)||enhancementContract(item)).map(item=>{const contract=enhancementContract(item),ownerRecord=enhancementOwnerRecord(item),record={title:item.title,text:item.text,value:ownerRecord?ownerRecord.points:item.value,detachment:det.title,tags:contract?.tags||item.tags||[],owner:contract?.owner||null,assignment:contract?.assignment||null};return ['chaos-space-marines','space-marines'].includes(config.id)?[enhancementRuleId(item),{...record,ruleId:enhancementRuleId(item),detachmentId:det.id}]:[titleKey(item.title),record]})));
