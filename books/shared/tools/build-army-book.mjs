...
const resolvedArmyRules=(config.armyRules||['Shadow in the Warp','Synapse']).map(title=>armyRuleSources.find(item=>titleKey(item.subject||item.title)===titleKey(title))).filter(Boolean).map(item=>({id:`army-rule-${slug(item.subject||item.title)}`,termId:config.armyRuleTermIds?.[item.subject||item.title],title:item.subject||item.title,text:item.change||item.summary||item.text,sourcePages:item.sourcePages,source:item.sourceBook?'dependency':item.sourcePages?'faction-pack':item.datasheetSource?'datasheet':'codex-parity',sourceBook:item.sourceBook,sourceFile:item.sourceFile,sourceVersion:item.sourceVersion}));
const resolvedUpdates=[...pack.updates.filter(item=>!resolvedArmyRules.some(rule=>rule.title===item.subject)).map(item=>({id:item.id,title:item.title||item.subject,text:item.summary||item.text||item.change,pages:item.sourcePages,type:'Official update'})),...pack.faqs.map(item=>({id:item.id,title:item.question,text:item.answer,pages:item.sourcePages,type:'Official FAQ'}))];
