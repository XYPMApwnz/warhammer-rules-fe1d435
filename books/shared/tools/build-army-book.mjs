...
for(const unit of units){
  for(const ability of unit.abilities||[])ability.termId=canonicalCoreAbilityTerms.get(coreBaseKey(ability.title))||addTerm(ability.title,ability.text,unit.id,'ability',unit.id);
  for(const ability of unit.wargearAbilities||[])ability.termId=canonicalCoreAbilityTerms.get(coreBaseKey(ability.title))||addTerm(ability.title,ability.text,unit.id,'ability',unit.id);
...
const bookAbilityTerms=new Map([...terms.values()].filter(term=>term.id.startsWith(`${config.id}-ability-`)).map(term=>[coreBaseKey(term.title),term.id]));
const weaponAbilityTokens=value=>`<div class="weapon-tags">${String(value).split(',').map(rawLabel=>{const label=rawLabel.trim(),termId=canonicalCoreAbilityTerms.get(coreBaseKey(label))||bookAbilityTerms.get(coreBaseKey(label));return termId?`<button class="tag" data-term="${termId}">${esc(label.toUpperCase())}</button>`:`<span class="tag">${esc(label.toUpperCase())}</span>`;}).join('')}</div>`;
...
const unitCard=unit=>{
  ...
  const parts={profile:`${base}-profile`,abilities:`${base}-abilities`,...((unit.wargearAbilities||[]).length?{'wargear abilities':wargearAbilitiesId}:{}),composition:`${base}-composition`,...((unit.relations?.leader||[]).length?{leader:`${base}-leader`}:{}),keywords:`${base}-keywords`};
  ...
  const leader=(unit.relations?.leader||[]).length?`<section class="unit-part" id="${parts.leader}" data-source-field="relations.leader"><h4>Leader</h4><p>This model can be attached to: ${unit.relations.leader.map(esc).join('; ')}.</p></section>`:'';
...
const normalizedHtml=html
  ...
  .replace('../shared/datasheet-layout.js"','../shared/datasheet-layout.js?v=3"')
