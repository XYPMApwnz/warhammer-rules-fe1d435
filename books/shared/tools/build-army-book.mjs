...
const heroMark=config.coverImage?`<div class="hero-mark hero-cover"><img src="./${esc(config.coverImage)}" width="800" height="1000" alt="${esc(config.title)} cover"></div>`:`<div class="hero-mark" aria-hidden="true"><span>${esc(config.factionKeyword)}</span></div>`;
...
let normalizedBookCss=(bookCss+forcedPhoneCss).replace('.unit-status{','.unit-status{white-space:pre-line;');
...
const generatedBookCss=config.dedicatedMobile?(config.coverImage?bookCss.replace(heroCoverBase,heroCoverWide):bookCss.replace(heroCoverBase,'').replace(heroCoverMobile,'')):normalizedBookCss;
