...
const stratagemCard=(item,det)=>`<article class="stratagem surface" data-rule-id="${esc(item.id)}" data-eligibility="${esc(JSON.stringify(stratagemEligibility(item)))}"><div class="stratagem-head"><div><h3><button class="term-button" data-term="${addTerm(item.title,`${item.when} ${item.target} ${item.effect}`,`detachment-${det.id}`,'stratagem')}">${esc(item.title)}</button></h3></div><div class="cp">${esc(item.cp)}CP</div></div><p class="field" data-source-field="when"><b>When</b><br>${esc(item.when)}</p><p class="field" data-source-field="target"><b>Target</b><br>${esc(item.target)}</p><p class="field" data-source-field="effect"><b>Effect</b><br>${esc(item.effect)}</p></article>`;
