...
const enhancementRuleId=item=>config.id==='chaos-space-marines'&&item.id?item.id:enhancementOwnerRecords.find(record=>titleKey(record.title)===titleKey(item.title))?.id||item.id||`enhancement-${slug(item.title)}`;
