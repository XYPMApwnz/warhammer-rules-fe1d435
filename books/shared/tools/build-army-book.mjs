...
const unitCard=unit=>{
  ...
  const ruleFacts={id:unit.id,unitId:unit.id,slug:base,keywords:unit.keywords||[],intrinsicKeywords:unit.keywords||[],abilities:[],termIds:[],epic:(unit.keywords||[]).some(item=>titleKey(item)==='epic hero'),deadlyDemise:false,attached:null,twoCharacters:null,warlord:null,candidates:candidates.get(unit.id)||[]};
...
const relatedInc=detachments.map(det=>`<section class="related-detachment" data-detachment="${esc(det.id)}" data-keyword-grants="${esc(JSON.stringify(relatedRules.keywordGrants?.[det.id]||[]))}"><h2>${esc(det.title)} <span class="detachment-dp">${esc(det.detachmentPoints)}DP</span></h2><div data-related-kind="stratagems">${(det.stratagems||[]).map(item=>stratagemCard(item,det)).join('')}</div><div data-related-kind="enhancements">${(det.enhancements||[]).map(item=>enhancementCard(item,det,{related:true})).join('')}</div></section>`).join('')+sharedCoreInc;
...
const normalizedHtml=html
  ...
  .replace('<script src="../shared/related-rules-matcher.js">','<script src="../shared/rule-facts.js?v=2"></script><script src="../shared/related-rules-matcher.js">')
