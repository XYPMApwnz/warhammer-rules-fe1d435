...
for(const unit of units){
  ...
  for(const ability of unit.abilities||[]){const key=coreBaseKey(ability.title),canonical=canonicalCoreAbilityTerms.get(key);ability.termId=canonical||config.armyRuleTermIds?.[ability.title]||addTerm(ability.title,ability.text,unit.id,'ability',unit.id,termScope);if(!canonical)scopedAbilityTerms.set(`${termScope}\0${key}`,ability.termId);}
  for(const ability of unit.wargearAbilities||[]){const key=coreBaseKey(ability.title),canonical=canonicalCoreAbilityTerms.get(key);ability.termId=canonical||config.armyRuleTermIds?.[ability.title]||addTerm(ability.title,ability.text,unit.id,'ability',unit.id,termScope);if(!canonical)scopedAbilityTerms.set(`${termScope}\0${key}`,ability.termId);}
