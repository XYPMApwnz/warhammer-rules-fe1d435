...
const officialMfm=config.sources.officialMfm?readJson(config.sources.officialMfm):null;
...
const officialEnhancementIdentityByPublication=new Map((officialMfm?.enhancements||[]).filter(item=>item.id).map(item=>{const matches=boundPointEnhancements.filter(point=>point.id===item.id);if(matches.length!==1)throw new Error(`${config.id}: official Enhancement identity ${item.id} must bind to one exact points record`);return[matches[0].id,item];}));
