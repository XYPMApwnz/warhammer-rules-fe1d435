...
import {createEffectivePointsProjection} from './effective-points-projection.mjs';
...
const pointsPublicationByEnhancement=new WeakMap(),pointsPublicationByDetachment=new WeakMap();
...
const localEnhancementPointsById=new Map((points.enhancements||[]).filter(item=>item.id).flatMap(item=>[[item.id,item],[String(item.id).replace(/^enhancement-/,''),item]]));
const enhancementPointsPublication=({det,item})=>pointsPublicationByEnhancement.get(item)||(det.dependencyBook?null:localEnhancementPointsById.get(item.id)||localEnhancementPointsById.get(enhancementRuleId(item)))||null;
const projectedEnhancementSources=projectionEnhancementSources.map(source=>({...source,publication:enhancementPointsPublication(source)})).filter(({item,ownerRecord,publication})=>Number.isFinite(Number(ownerRecord?.points??publication?.value??item.value)));
const effectivePointsProjection=createEffectivePointsProjection({
  ...
  units:units.map(unit=>({id:unit.id,title:unit.title,sourceBookId:unit.dependencyBook||config.id,publicationState:unit.status||'Current',points:unit.points||[],paidWargear:unit.paidWargear||[],...(unit.pointsSource?{pointsSource:unit.pointsSource}:{}),...(unit.compatibleChapterKeywords?.length?{compatibleChapterKeywords:unit.compatibleChapterKeywords}:{}),ruleProfile:unitRuleProfiles.get(unit.id),publicationRecord:unitPointsPublication(unit)||{id:unit.id,title:unit.title,status:unit.status,sourceLayer:unit.sourceLayer,points:unit.points,paidWargear:unit.paidWargear,pointsSource:unit.pointsSource}})),
  detachments:detachments.map(det=>({id:det.id,title:det.title,sourceBookId:det.dependencyBook||config.id,detachmentPoints:Number(String(det.detachmentPoints??det.dp??0).match(/\d+/)?.[0]||0),forceDisposition:det.forceDisposition||det.disposition||'',publicationRecord:pointsPublicationByDetachment.get(det)||{title:det.title,detachmentPoints:det.detachmentPoints,forceDisposition:det.forceDisposition}})),
  enhancements:projectedEnhancementSources.map(({det,item,contract,ownerRecord,publication})=>({id:enhancementRuleId(item),sourceId:item.sourceId||null,ruleId:enhancementRuleId(item),legacyKey:item.legacyKey||null,detachmentId:det.id,detachmentTitle:det.title,sourceBookId:det.dependencyBook||config.id,title:publication?.title||item.title,value:Number(ownerRecord?.points??publication?.value??item.value),owner:contract?.owner||item.owner||null,assignment:contract?.assignment||item.assignment||null,tags:contract?.tags||item.tags||[],text:publication?.text??item.text??'',...(publication?.profile||item.profile?{profile:publication?.profile||item.profile}:{}),...(item.sourceLimited?{sourceLimited:true}:{}),publicationRecord:publication||{id:item.id,title:item.title,detachment:det.title,value:item.value,text:item.text,profile:item.profile,pointsSource:item.pointsSource}}))
});
