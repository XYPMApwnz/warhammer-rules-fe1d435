...
const enrichEnhancement=(item,detachment)=>{const qualified=Boolean(config.detachmentQualifiedEnhancementPoints),current=qualified?enhancementPointsByDetachment.get(`${titleKey(detachment)}\0${pointTitleKey(item.title)}`):enhancementPointsByTitle.get(pointTitleKey(item.title));if(!current)return item;const sourceId=item.sourceId||(qualified&&current.id?item.id:null);return {...item,...(qualified&&current.id?{id:current.id}:{}),...(sourceId?{sourceId}:{}),value:current.value,pointsSource:current.pointsSource,...enhancementDependencyFacts(item,qualified?current:null)};};
...
for(const unit of units){
  ...
  for(const ability of unit.abilities||[]){const key=coreBaseKey(ability.title),canonical=canonicalCoreAbilityTerms.get(key);ability.termId=canonical||config.armyRuleTermIds?.[ability.title]||addTerm(ability.title,sourceAwareText(ability),unit.id,'ability',unit.id,termScope);if(!canonical)scopedAbilityTerms.set(`${termScope}\0${key}`,ability.termId);}
  for(const ability of unit.wargearAbilities||[]){const key=coreBaseKey(ability.title),canonical=canonicalCoreAbilityTerms.get(key);ability.termId=canonical||config.armyRuleTermIds?.[ability.title]||addTerm(ability.title,sourceAwareText(ability),unit.id,'ability',unit.id,termScope);if(!canonical)scopedAbilityTerms.set(`${termScope}\0${key}`,ability.termId);}
...
const rosterEnhancements=Object.fromEntries(detachments.flatMap(det=>(det.enhancements||[]).filter(item=>!enhancementOwnerRecord(item)||enhancementContract(item)).map(item=>{const contract=enhancementContract(item),ownerRecord=enhancementOwnerRecord(item),dependencyRecord=Boolean(det.dependencyBook),sourceId=dependencyRecord?item.sourceId:null,facts=dependencyRecord?enhancementDependencyFacts(item):{},record={title:item.title,text:item.text,value:ownerRecord?ownerRecord.points:item.value,detachment:det.title,tags:contract?.tags||item.tags||[],...(sourceId?{sourceId}:{}),...(item.profile?{profile:item.profile}:{}),...facts,owner:contract?.owner||facts.owner||null,assignment:contract?.assignment||facts.assignment||null};return ['chaos-space-marines','space-marines','dark-angels','blood-angels'].includes(config.id)?[enhancementRuleId(item),{...record,ruleId:enhancementRuleId(item),detachmentId:det.id}]:[titleKey(item.title),record]})));
