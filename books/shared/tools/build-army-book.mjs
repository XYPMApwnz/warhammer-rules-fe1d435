...
const categoriesFor=(sourceUnits,prefix='')=>[...fixedCategoryOrder,...new Set(sourceUnits.map(unit=>unit.category).filter(category=>!fixedCategoryOrder.includes(category)&&category!=='Other'&&category!=='Warhammer Legends')),'Other','Warhammer Legends'].map(title=>({title,id:`datasheets-${prefix?`${prefix}-`:''}${slug(title)}`,units:sourceUnits.filter(unit=>unit.category===title)})).filter(group=>group.units.length);
...
const relationLabel=name=>{const target=unitByTitle.get(titleKey(name));return groupedDependencyDatasheets&&target&&renderedUnitIds.has(target.id)?`<button class="term-button" data-journey-target="${esc(target.id)}" data-journey-type="datasheet">${esc(name)}</button>`:esc(name);};
...
const unitCard=unit=>{
  ...
  const compact=(unit.dependencyCompactSharedAbilities||config.compactSharedAbilities||[]).map(titleKey),isCompact=item=>compact.some(value=>titleKey(item.title)===value||titleKey(item.title).startsWith(`${value} `));
...
const appJs=`try{window.WHArmyBook.install(${JSON.stringify({bookId:config.id,readerPath:'./reader.html',...(config.dedicatedMobile||config.generatedMobile?{dedicatedMobile:true}:{})})});}catch(error){document.documentElement.dataset.bookError=String(error&&error.stack||error);console.error(error);}\n`;
