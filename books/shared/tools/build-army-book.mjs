...
const stratagemEligibility=item=>{
  ...
  for(const role of roles)inspect(role.selector||role);
  return explicit;
};
...
for(const det of pack.detachments){
  ...
  for(const item of det.stratagems)addTerm(item.title,`${item.when} ${item.target} ${item.effect}`,`detachment-${det.id}`,'stratagem');
}
...
const unitSourceState=unit=>verificationBuild?`<div class="unit-source-state"><span>${config.dedicatedMobile?'Datasheet structure · pinned 11e community catalogue':'Datasheet structure · current 11e catalogue'}</span><span>${unit.pointsSource?`${esc(unit.pointsSource.label)} · checked ${esc(unit.pointsSource.verifiedAt)}`:'Points · catalogue snapshot'}</span>${unit.wargearSource?`<span>${config.dedicatedMobile?'Wargear & composition · pinned 11e community transcription':'Wargear & composition · current 11e reference'}</span>`:''}${unit.sourcePages?`<span>Official Faction Pack overlay · p. ${unit.sourcePages.join('–')}</span>`:''}</div>`:'';
const armyRules=['Shadow in the Warp','Synapse'].map(title=>pack.updates.find(item=>titleKey(item.subject||item.title)===titleKey(title))).filter(Boolean).map(item=>({id:`army-rule-${slug(item.subject||item.title)}`,title:item.subject||item.title,text:item.change||item.summary||item.text,sourcePages:item.sourcePages}));
...
const toc=navLeaf('start','Start',1)
  ...
  +navBranch('detachments','Detachments',1,detachments.map(det=>navLeaf(`detachment-${det.id}`,det.title,2)).join(''))
...
const detachmentHtml=detachments.map(det=>{
  ...
  const rule=det.rule?`<article class="rule-card surface"><h4><button class="term-button" data-term="${addTerm(det.rule.title,det.rule.text,`detachment-${det.id}`,'detachment-rule')}">${esc(det.rule.title)}</button></h4><p>${esc(det.rule.text).replace(/\n/g,'<br>')}</p></article>`:`<article class="rule-card surface source-warning"><h4>Codex source required</h4><p>This Detachment is current, but its complete rule and Stratagem text is not present in Faction Pack v${esc(pack.meta.version)}. It is not reproduced here until the Codex layer is verified.</p></article>`;
  return tracked(`detachment-${det.id}`,det.title,`<div class="detachment-meta"><span>${official?'OFFICIAL FACTION PACK':'CODEX INDEX'}</span>${det.forceDisposition?`<span>${esc(det.forceDisposition)}</span>`:''}${det.detachmentPoints?`<span>${esc(det.detachmentPoints)}DP</span>`:''}</div>${rule}${enhancements?`<h4 class="subheading">Enhancements</h4><div class="detachment-grid">${enhancements}</div>`:''}${stratagems?`<h4 class="subheading">Stratagems</h4><div class="detachment-grid">${stratagems}</div>`:''}${official?`<p class="source">${sourceLink(det.sourcePages)}</p>`:''}`,'content-group detachment');
}).join('');
const statline=unit=>(unit.profiles||[]).map(profile=>`<div class="model-profile">${unit.profiles.length>1?`<h5>${esc(profile.name)}</h5>`:''}<div class="statline">${Object.entries(profile.stats).filter(([,value])=>value).map(([name,value])=>`<div class="stat"><b>${esc(name)}</b><span>${esc(value)}</span></div>`).join('')}</div></div>`).join('');
const weaponTables=unit=>['ranged','melee'].map(mode=>{const rows=(unit.weapons||[]).filter(item=>item.mode===mode);if(!rows.length)return'';const skill=mode==='ranged'?'BS':'WS';return`<div class="weapon-group"><h5>${mode==='ranged'?'Ranged':'Melee'} weapons</h5><div class="weapon-table" role="table"><div class="weapon-row weapon-head"><div>Weapon</div><div>Range</div><div>A</div><div>${skill}</div><div>S</div><div>AP</div><div>D</div></div>${rows.map(item=>`<div class="weapon-row"><div><button class="weapon-button" data-term="${item.termId}">${esc(item.name)}</button>${item.abilities?`<small>${esc(item.abilities)}</small>`:''}</div><div data-label="Range">${esc(item.range)}</div><div data-label="A">${esc(item.a)}</div><div data-label="${skill}">${esc(item.skill)}</div><div data-label="S">${esc(item.s)}</div><div data-label="AP">${esc(item.ap)}</div><div data-label="D">${esc(item.d)}</div></div>`).join('')}</div></div>`;}).join('');
const unitCard=unit=>{
  ...
  const wargear=unit.wargear?.length?`<h5>Wargear Options</h5><ul>${unit.wargear.map(item=>`<li>${esc(item)}</li>`).join('')}</ul>`:'';
  return`<article class="unit-card surface${unit.status==='Warhammer Legends'?' legends-card':''}" id="${unit.id}" data-track="${unit.id}" data-unit-title="${esc(unit.title)}" data-keywords="${esc((unit.keywords||[]).join('|'))}" data-related-candidates="${esc(JSON.stringify(candidates.get(unit.id)||[]))}"><div class="unit-header"><div><div class="eyebrow">${esc(unit.status)} · ${esc(unit.sourceLayer)}</div><h3>${esc(unit.title)}</h3></div><div class="unit-status">${esc(pointsText||'POINTS PENDING')}</div></div>${unitSourceState(unit)}<div class="local-nav">${tabs}</div><section class="unit-part" id="${parts.profile}"><h4>Profile & Weapons</h4>${statline(unit)}${weaponTables(unit)}</section><section class="unit-part" id="${parts.abilities}"><h4>Abilities</h4>${shared.length?`<div class="keyword-list shared-abilities">${shared.map(item=>`<button class="term-button" data-term="${item.termId}">${esc(item.title)}</button>`).join('')}</div>`:''}<div class="ability-list">${specific.map(item=>`<article class="ability"><h5><button class="term-button" data-term="${item.termId}">${esc(item.title)}</button></h5>${item.text?`<p>${esc(item.text)}</p>`:''}</article>`).join('')}</div></section><section class="unit-part" id="${parts.composition}"><h4>Composition & Wargear</h4>${composition}${wargear}${unit.paidWargear?.length?`<h5>Paid wargear</h5><ul>${unit.paidWargear.map(item=>`<li>${esc(item.name)} · +${item.value} pts</li>`).join('')}</ul>`:''}</section><section class="unit-part" id="${parts.keywords}"><h4>Keywords</h4><div class="keyword-list">${(unit.keywords||[]).map(item=>`<span>${esc(item)}</span>`).join('')}</div></section>${unit.sourcePages?`<p class="source">${sourceLink(unit.sourcePages)}</p>`:''}</article>`;
};
const datasheetHtml=categories.map(group=>tracked(group.id,group.title,group.units.map(unitCard).join(''))).join('');
const armyRulesHtml=armyRules.map(item=>tracked(item.id,item.title,`<article class="rule-card surface"><div class="eyebrow">Army rule</div><p>${esc(item.text)}</p><p class="source">${sourceLink(item.sourcePages)}</p></article>`)).join('');
...
const normalizedHtml=html
  ...
  .replace('<section class="section" id="updates"',`<section class="section" id="army-rules" data-track="army-rules"><h2 class="section-title">Army Rules</h2>${armyRulesHtml}</section><section class="section" id="updates"`)
  ...
  .replace('../shared/popup-content.js"','../shared/popup-content.js?v=2"')
  .replace('../shared/glossary-autolink.js"','../shared/glossary-autolink.js?v=8"')
  .replace('../shared/related-rules-matcher.js"','../shared/related-rules-matcher.js?v=1"')
  ...
  .replace('../shared/army-related-rules.js','../shared/army-related-rules.js?v=4')
...
const indexHtml='<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'+esc(config.title)+'</title></head><body><script>location.replace(new URL("./reader.html"+location.search+location.hash,location.href))<\/script><a href="./reader.html">Open '+esc(config.title)+' Rules</a></body></html>\n';
...
const outputs=new Map([
  ['reader.html',normalizedHtml],['index.html',indexHtml],['scripts/data.js',dataJs],['scripts/app.js',appJs],['styles/book.css',normalizedBookCss],...(config.dedicatedMobile?[['mobile/related-rules.source.inc',relatedInc+'\n']]:[['mobile/index.html',mobileIndex],['mobile/related-rules.inc',relatedInc+'\n']])
]);
const errors=[];
