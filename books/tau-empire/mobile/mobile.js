(function(){
  ...
  function showTerm(trigger){
    ...
    full.href=`../../../glossary/index.html#${encodeURIComponent(id)}`;const path=trigger.dataset.mobileRulePath||trigger.dataset.fullRulePath;rule.hidden=!path;if(path)rule.href=window.WHGlossaryReturn.href(path);if(!dialog.open)dialog.showModal();
  }
  ...
  function restore(record,{push=false}={}){const all=triggers(),trigger=all[record.triggerIndex]?.dataset.term===record.termId?all[record.triggerIndex]:all.find(node=>node.dataset.term===record.termId);history[push?'pushState':'replaceState'](history.state,'',record.path);scrollTo(record.scrollX||0,record.scrollY||0);requestAnimationFrame(()=>{if(trigger)showTerm(trigger);window.WHGlossaryReturn.clear();returnPopup.hidden=true;});}
  ...
  function filter(){
    if(!loaded||!unit)return;const profile=window.WHArmyRelatedRules.profile(unit);
  ...
  full.addEventListener('click',()=>save('automatic'));rule.addEventListener('click',()=>{save('manual');dialog.close();setTimeout(syncReturn,0);});returnPopup.addEventListener('click',event=>{const record=window.WHGlossaryReturn.read();if(!record)return;if(window.WHGlossaryReturn.isSameDocument(record)){event.preventDefault();restore(record,{push:true});}else window.WHGlossaryReturn.setRestoreMode('automatic');});
  select?.addEventListener('change',()=>{try{localStorage.setItem('tau-empire-detachment-filter',select.value);}catch{}filter();});try{const saved=localStorage.getItem('tau-empire-detachment-filter');if(saved&&select?.querySelector(`option[value="${CSS.escape(saved)}"]`))select.value=saved;}catch{}
