...
const report=JSON.parse(fs.readFileSync(path.join(root,'books/tau-empire/reports/army-book-blueprint-conformance.json'),'utf8'));
...
assert.deepEqual(report.summary.sourceAuthorityBlockers,report.requirements.filter(item=>item.blockerKind==='source-authority').map(item=>item.id));
assert.deepEqual(report.summary.publicationBlockers,report.requirements.filter(item=>item.blockerKind==='publication').map(item=>item.id));
assert.deepEqual(report.summary.externalBlockers,report.requirements.filter(item=>item.blockerKind==='external').map(item=>item.id));
assert.equal(report.summary.partial>0,true,'T’au Blueprint conformance must remain partial while authoritative source/publication gaps exist');

const source=fs.readFileSync(path.join(root,'books/tau-empire/mobile/phone-popup-controller.js'),'utf8');
const {stratagemTypes}=await import(new URL('../scripts/stratagem-types.mjs',import.meta.url));
assert.equal(stratagemTypes.size,31);
assert.equal([...stratagemTypes.values()].filter(type=>type==='unknown').length,7);
assert.equal([...stratagemTypes.values()].filter(type=>type!=='unknown').length,24);
...
const State=context.window.TAUPhonePopupState;
...
console.log(`PASS T'au Empire Blueprint report ${report.requirementCount.total}/${report.requirementCount.unique}; popup state semantics`);
