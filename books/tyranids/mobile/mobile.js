(async function(){
  ...
  function rosterContext(){if(!rosterMode)return null;let record;try{record=(JSON.parse(localStorage.getItem('wh40k-rosters-v1'))||[]).find(item=>item?.id===params.get('roster'));}catch{}if(!record)return null;let roster=record.roster;if(record.sourceText&&window.WHRosterParser){try{const parsed=window.WHRosterParser.parse(record.sourceText);if(parsed?.units?.length)roster=parsed;}catch{}}if(normalize(roster?.faction)!=='tyranids'||!Array.isArray(roster?.units)||!roster.units.length)return null;const canonicalUnits=new Set([...nav.querySelectorAll('.mobile-unit-groups a')].map(link=>normalize(link.textContent))),canonicalDetachments=new Set([...nav.querySelectorAll('.phone-tree > details:first-of-type .mobile-nav-branch > a')].map(routeDetachmentId));if(!roster.units.some(item=>canonicalUnits.has(normalize(item.name))))return null;const matching=roster.units.filter(item=>normalize(item.name)===normalize(unit?.dataset.unitTitle));if(unit&&!matching.length)return null;const owners=new Set(matching.map(item=>item.id)),enhancements=new Set((roster.enhancements||[]).filter(item=>item.ownerStatus==='resolved'&&owners.has(item.ownerUnitId)).map(item=>normalize(item.name))),detachments=[...new Set((roster.detachments?.length?roster.detachments.map(item=>item.label):[roster.detachment]).filter(Boolean).map(slug))];return detachments.length===1&&canonicalDetachments.has(detachments[0])?{roster,detachments,enhancements}:null;}
