(async function(){
  ...
  function rosterContext(){if(!rosterMode)return null;let record;try{record=(JSON.parse(localStorage.getItem('wh40k-rosters-v1'))||[]).find(item=>item?.id===params.get('roster'));}catch{}if(!record)return null;let source=record.roster;if(record.sourceText&&window.WHRosterParser){const parsed=window.WHRosterParser.parse(record.sourceText);if(parsed.units.length)source=parsed;}if(!tyranidsFaction(source?.faction))return null;const units=(source.units||[]).filter(item=>normalize(item.name)===normalize(unit?.dataset.unitTitle)),selected=new Set((source.units||[]).map(item=>normalize(item.name))),owners=new Set(units.map(item=>item.id)),enhancements=new Set((source.enhancements||[]).filter(item=>item.ownerStatus==='resolved'&&owners.has(item.ownerUnitId)).map(item=>normalize(item.name))),detachments=[...new Set((source.detachments?.length?source.detachments.map(item=>item.label):[source.detachment]).filter(Boolean).map(slug))];return detachments.length?{detachments,enhancements,selected,source,units}:null;}
...
if(rosterMode){
  ...
  if(unit)window.WHBookRosterEnhancements?.decorate(unit,roster.source,roster.units);
}
