(async function(){
  ...
  function decorateStratagemTypes(root){
    root.querySelectorAll('.stratagem').forEach(card=>{if(/^(battle-tactic|strategic-ploy|wargear|epic-deed|core|unknown)$/.test(card.dataset.stratagemType||''))return;const id=card.dataset.ruleId||card.id,mapped=stratagemTypes.get(id)||stratagemTypes.get(id.replace(/^stratagem-/,'')),match=card.querySelector('.stratagem-type')?.textContent.trim().match(/(Battle Tactic|Strategic Ploy|Wargear|Epic Deed|Core) Stratagem\s*$/i);card.dataset.stratagemType=mapped||(match?match[1].toLowerCase().replace(/\s+/g,'-'):'unknown');});
  }
