...
assert.deepEqual(report.summary,{associations:{core:328,faction:1086},boardingActionNamesIgnored:21,coreStratagems:{canonical:10,observed:9},datasheets:{canonical:49,imported:49},factionStratagems:{canonical:51,observed:51},unresolved:0});
assert.equal(Object.keys(snapshot.units).length,49);assert.equal(Object.keys(snapshot.coreUnits).length,49);
assert.deepEqual(generated,built,'generated matrix is stale');
const rows=Object.values(generated.units).flat();
assert.equal(rows.length,1694);assert.equal(rows.filter(row=>row.scope==='core').length,328);assert.equal(rows.filter(row=>row.kind==='enhancement').length,194);assert.equal(rows.filter(row=>row.detachmentId&&row.kind!=='enhancement').length,1172);assert.equal(rows.filter(row=>row.state==='conditional').length,90);
...
console.log('Tyranids Compatible Rules QA passed: 1694 matrix rows, 90 conditional, legacy matcher absent.');
