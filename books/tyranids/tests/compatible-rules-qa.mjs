...
assert.equal(rows.length,1743);assert.equal(rows.filter(row=>row.scope==='core').length,335);assert.equal(rows.filter(row=>row.kind==='enhancement').length,213);assert.equal(rows.filter(row=>row.detachmentId&&row.kind!=='enhancement').length,1195);assert.equal(rows.filter(row=>row.state==='conditional').length,90);
...
console.log('Tyranids Compatible Rules QA passed: 1743 matrix rows, 90 conditional, legacy matcher absent.');
