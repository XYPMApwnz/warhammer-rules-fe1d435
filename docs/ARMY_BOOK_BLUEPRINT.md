...
- Level: MUST
- Normative requirement: Missing, wrong-faction, corrupt, empty, unresolved, route-incompatible, or ambiguous roster data MUST fail closed and MUST NOT expose roster-scoped or non-deterministic results. On any Army Book route carrying a roster query, the complete roster context MUST be resolved and validated before roster-dependent navigation filtering, Compatible Rules, roster-specific popup or history state, roster presentation, roster globals, or any other roster-mode side effect is activated. A valid roster context MUST establish an existing usable roster record, the correct faction, a non-empty unit inventory, exactly one canonical Detachment, at least one roster unit that resolves to a canonical datasheet in the current Army Book, and, on a route representing one specific datasheet, at least one roster unit instance that resolves to that current route. If validation fails, runtime MUST replace the current history entry with the canonical Roster Guides route by using location.replace(...) and MUST immediately terminate Army Book initialization for that route. Silently removing only roster-sensitive controls while leaving the Army Book active under an invalid roster query MUST NOT satisfy this requirement. For AB-ROSTER-004 only, the accepted T’au Empire Phone production control flow — resolve the roster context, then perform location.replace(...) followed immediately by return when roster mode is active and the context is invalid — is the canonical technical reference. This explicit alternate canonical reference is permitted by AB-SCOPE-004 and AB-SCOPE-007 and supersedes the legacy Death Guard soft-failure path for roster failure handling.
- Owner: shared Army Book capability
- Applicability: all Army Books
- Required evidence: Static-source inspection of the accepted T’au Empire Phone roster validation and fail-closed control flow; a per-book implementation table covering desktop and Phone validation order, canonical datasheet resolution, redirect destination, immediate termination, and roster-side-effect boundary; executable invalid-roster fixtures for missing record, corrupt storage, wrong faction, missing or empty units, zero canonical datasheet matches, missing or ambiguous Detachment, and route-incompatible unit context where applicable; real-browser navigation evidence proving location replacement to the canonical Roster Guides route, replacement-history semantics, and absence of roster-scoped initialization; valid-roster regression evidence proving that correct roster routes remain active and preserve deterministic Detachment, unit, loadout, and ownerUnitId behavior.
