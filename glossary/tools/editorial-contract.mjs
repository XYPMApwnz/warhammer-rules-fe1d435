...
export function validateEditorialContract(contract,{knownTermIds,knownTerms}={}){
  const identities=knownTermIds||new Set(knownTerms?.keys?.()||[]);
  ...
  for(const record of contract.summaries){
    ...
    if(knownTermIds||knownTerms)assert(identities.has(record.termId),`unknown canonical term ID ${record.termId}`);
    if(knownTerms)assert(knownTerms.get(record.termId).canonicalSource?.documentId===record.semanticSource.documentId,`${record.termId} conflicts with its canonical semantic owner`);
...
export function loadEditorialContract({glossaryRoot=defaultGlossaryRoot,authenticateRepository=true,knownTermIds,knownTerms}={}){
  ...
  const contract=validateEditorialContract(JSON.parse(fs.readFileSync(artifactPath,'utf8')),{knownTermIds,knownTerms});
