(() => {
  ...
  const css=`@media (max-width:480px){.unit-card > .unit-part{padding-top:12px!important;padding-bottom:14px!important}.unit-card > .unit-part > h4{font-size:20px!important;margin-bottom:8px!important}}`; 
  let style;
  function apply(){const doc=frame.contentWindow?.document;if(!doc)return;const mode=document.documentElement.dataset.mode;doc.documentElement.setAttribute('data-lab-mode',mode);if(style){style.remove();style=null}if(mode==='experimental'){style=doc.createElement('style');style.textContent=css;doc.head.appendChild(style)}toggle.textContent=mode==='experimental'?'Current spacing':'Experimental spacing'}
