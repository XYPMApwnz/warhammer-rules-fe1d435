...
const inventory=Object.freeze({
  'death-guard':Object.freeze({migrated:['typhus-ability-the-destroyer-hive','lord-of-poxes-ability-shroud-of-disease','lord-of-virulence-ability-virulent-aura','noxious-blightbringer-ability-sickening-vitality','tallyman-ability-malicious-calculations','biologus-putrifier-ability-foul-infusion'],deterministic:['deathshroud-terminators-ability-silent-bodyguard'],ambiguous:['enhancement-witherbone-pipes','enhancement-vile-vigour','enhancement-helm-of-the-fly-king']}),
...
try{
  ...
  const dgCatalog=loadCatalog('death-guard'),malicious=ability(dgCatalog,'tallyman-ability-malicious-calculations');
...
• 1x Plague Champion: Boltgun, Plague knives`;
  ...
  const foul=ability(dgCatalog,'biologus-putrifier-ability-foul-infusion');
...
10x Fulgurite Electro-Priests (140 pts)`,attachments:{'parsed-unit-2':['parsed-unit-1'],'parsed-unit-5':['parsed-unit-4']}};
  ...
  const electroTarget=await open('adeptus-mechanicus',amRecord,'parsed-unit-4','unit-tech-priest-dominus'),electroResult=await inspectReference(electroTarget.page,'parsed-unit-4',electro.id);assert.equal(electroResult.effect?.source?.ownerInstanceId,'parsed-unit-5');assert.equal(electroResult.articleCount,1);assert.equal(electroResult.title,electro.title);assert.equal(electroResult.text,electro.text);assert.equal(electroResult.source,'Fulgurite Electro-Priests');assert.doesNotMatch(electroResult.activeText,/Electro-Infusion/);await electroTarget.context.close();
