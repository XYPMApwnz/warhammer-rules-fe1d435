...
assert.equal(Object.values(inventory).reduce((count,book)=>count+book.migrated.length,0),8);
...
for(const key of ['destroyer','shroud','virulent','vitality','malicious'])assert.match(dgSource,new RegExp(`canonicalAbility\\(DG_RULE\\.${key}`),`DG ${key} must use canonical reference`);
...
assert.match(dgSource,/ability\('foul-infusion'/,'Foul Infusion deterministic Critical Hit result must remain');
...
10x Fulgurite Electro-Priests (140 pts)`,attachments:{'parsed-unit-2':['parsed-unit-1'],'parsed-unit-5':['parsed-unit-4']}};
  ...
  console.log('Attached canonical Ability reference inventory/browser QA: PASS (9 books; 8 migrated).');
