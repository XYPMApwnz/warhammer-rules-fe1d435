...
try{
  ...
  try{
    ...
    await page.evaluate(()=>localStorage.setItem('wh40k-rosters-v1',JSON.stringify([{id:'am-owner-filter',name:'Adeptus Mechanicus test roster',roster:{faction:'Adeptus Mechanicus',detachment:'Cohort Cybernetica',detachments:[{label:'Cohort Cybernetica'}],declared:95,units:[{id:'am-owner-1',name:'Tech-Priest Enginseer',quantity:1,points:75}],enhancements:[{name:'Necromechanic',ownerUnitId:'am-owner-1',ownerStatus:'resolved'}]}},{id:'am-unverified-label',roster:{faction:'Adeptus Mechanicus',detachment:'Cohort Acquisitus',detachments:[{label:'Cohort Acquisitus'}],declared:75,units:[{id:'am-unverified-owner',name:'Tech-Priest Enginseer',quantity:1,points:75}],enhancements:[]}}])));
    ...
    await page.goto(`${origin}/books/adeptus-mechanicus/reader.html?roster=am-unverified-label#unit-tech-priest-enginseer`);
    await page.locator('#unit-tech-priest-enginseer .related-rules-trigger').click();
    const desktopUnverified=page.locator('.related-rules-layer [data-rule-id="stratagem-repolarised-augurs"]:visible');await desktopUnverified.waitFor();
    ...
    const ordinaryUnverified=page.locator('#relatedRulesContent [data-rule-id="stratagem-repolarised-augurs"]:visible');assert.equal(await ordinaryUnverified.getAttribute('data-stratagem-type'),'unknown','Mechanicus ordinary Phone view must preserve unverified type metadata');assert.deepEqual(await ordinaryUnverified.locator('.stratagem-type').allTextContents(),['Type unverified'],'Mechanicus ordinary Phone view must show one honest unverified label');
    ...
    await page.evaluate(()=>{const records=JSON.parse(localStorage.getItem('wh40k-rosters-v1'));records.push(
    ...
    const expectMechanicusRosterRejected=async(id,route,label)=>{
      ...
      const trace=[];let rejectLimit;const navigationLimit=new Promise((_,reject)=>{rejectLimit=reject;});
      const onNavigation=frame=>{if(frame===page.mainFrame()){trace.push(frame.url());if(trace.length>4)rejectLimit(new Error(label+': navigation limit exceeded: '+trace.join(' -> ')));}};
      page.on('framenavigated',onNavigation);
      try{await Promise.race([(async()=>{const target=new URL(route,origin);target.searchParams.set('roster',id);await page.goto(target.href);await page.waitForURL(url=>url.pathname==='/roster-guides/index.html'&&!url.searchParams.has('roster'));await page.locator('#roster-form').waitFor();})(),navigationLimit]);}finally{page.off('framenavigated',onNavigation);}
      assert.equal(new URL(page.url()).pathname,'/roster-guides/index.html',label+': neutral Roster Guides destination');
      assert.equal(new URL(page.url()).searchParams.has('roster'),false,label+': rejected roster must not become an auto-open parameter');
      assert.equal(trace.slice(trace.findIndex(url=>new URL(url).pathname==='/roster-guides/index.html')+1).some(url=>new URL(url).pathname.startsWith('/books/adeptus-mechanicus/')),false,label+': Army Book must not reopen after failure handoff');
      for(const selector of ['#relatedRules','.unit-card','.related-rules-trigger','[data-roster-active]'])assert.equal(await page.locator(selector).count(),0,label+': '+selector+' absent at terminal destination');
  ...
  try{
    ...
    const expectTyranidsPhoneRosterRejected=async label=>{
      await page.waitForURL('**/roster-guides/index.html*');
      assert.equal(new URL(page.url()).pathname,'/roster-guides/index.html',`${label}: canonical Roster Guides destination`);
      for(const selector of ['#relatedRules','#relatedRulesContent','.unit-card','.related-rules-trigger','[data-roster-active]'])assert.equal(await page.locator(selector).count(),0,`${label}: ${selector} must be absent after replacement`);
    };
    await expectTyranidsPhoneRosterRejected('wrong-faction Tyranids Phone roster');
    await page.goto(`${origin}/index.html?tyr-phone-roster-control=1`);
    await page.goto(`${origin}/books/tyranids/mobile/hive-tyrant.html?roster=missing-tyr-history`);
    await expectTyranidsPhoneRosterRejected('Tyranids Phone replacement-history fixture');
    await page.goBack();
    await page.waitForURL(url=>url.pathname.endsWith('/index.html')&&url.search==='?tyr-phone-roster-control=1');
    ...
    await page.goto(`${origin}/books/tyranids/mobile/hive-tyrant.html?roster=tyr-multiple-detachments`);await expectTyranidsPhoneRosterRejected('multiple-Detachment Tyranids Phone roster');
    await page.goto(`${origin}/books/tyranids/mobile/hive-tyrant.html?roster=missing-roster`);
    await expectTyranidsPhoneRosterRejected('missing Tyranids Phone roster');
    await page.evaluate(()=>localStorage.setItem('wh40k-rosters-v1','{broken'));
    ...
    await page.goto(`${origin}/books/tyranids/mobile/hive-tyrant.html?roster=broken`);
    await expectTyranidsPhoneRosterRejected('corrupt Tyranids Phone roster');
  ...
  const assertTypePalette=(cards,label,requireAll=true,requireUnverified=false)=>{const names={'battle-tactic':'Battle Tactic Stratagem','strategic-ploy':'Strategic Ploy Stratagem',wargear:'Wargear Stratagem','epic-deed':'Epic Deed Stratagem',core:'Core Stratagem'},known=cards.filter(card=>canonicalStratagemTypes.includes(card.type)),palette=Object.fromEntries(canonicalStratagemTypes.map(type=>[type,known.find(card=>card.type===type)?.color])),rgb=hex=>{const value=hex.slice(1);return `rgb(${parseInt(value.slice(0,2),16)}, ${parseInt(value.slice(2,4),16)}, ${parseInt(value.slice(4,6),16)})`;};if(requireAll)for(const type of canonicalStratagemTypes)assert.ok(palette[type],`${label} lacks ${type}`);for(const card of cards){assert.ok([...canonicalStratagemTypes,'unknown'].includes(card.type),`${label} invalid type: ${JSON.stringify(card)}`);assert.equal(card.typeLabelCount,1,`${label} must expose one type label: ${JSON.stringify(card)}`);if(card.rendered)assert.equal(card.typeLabelVisible,true,`${label} type label is not visible: ${JSON.stringify(card)}`);if(card.type==='unknown'){if(requireUnverified)assert.equal(card.typeLabel,'Type unverified',`${label} unknown type is misleading: ${JSON.stringify(card)}`);}else assert.ok(card.typeLabel.toLowerCase().endsWith(names[card.type].toLowerCase()),`${label} visible type disagrees with metadata: ${JSON.stringify(card)}`);assert.ok(card.turn&&card.classes.includes(card.turn==='THEIR TURN'?'turn-their':card.turn==='YOUR TURN'?'turn-yours':'turn-any'),`${label} lost turn metadata: ${JSON.stringify(card)}`);assert.equal(card.turnLabel,card.turn,`${label} turn label mismatch: ${JSON.stringify(card)}`);assert.equal(card.railDisplay,'block',`${label} type rail is hidden: ${JSON.stringify(card)}`);assert.ok(card.railWidth>0,`${label} type rail has no width: ${JSON.stringify(card)}`);assert.equal(card.headerBorder,rgb(card.color),`${label} header accent disagrees with type color: ${JSON.stringify(card)}`);assert.equal(card.cpBorder,rgb(card.color),`${label} CP accent disagrees with type color: ${JSON.stringify(card)}`);}assert.equal(new Set(Object.values(palette).filter(Boolean)).size,Object.values(palette).filter(Boolean).length,`${label} type colors must be distinct`);for(const type of canonicalStratagemTypes){const group=known.filter(card=>card.type===type);if(new Set(group.map(card=>card.turn)).size>1)assert.equal(new Set(group.map(card=>card.color)).size,1,`${label} ${type} color changes with turn`);}const sameTurnPair=known.find((card,index)=>known.slice(index+1).some(other=>other.turn===card.turn&&other.type!==card.type));if(requireAll)assert.ok(sameTurnPair&&known.some(other=>other.turn===sameTurnPair.turn&&other.type!==sameTurnPair.type&&other.color!==sameTurnPair.color),`${label} same-turn different types must differ`);return palette;};
  ...
  try{
    ...
    const inspectTypes=selector=>page.locator(selector).evaluateAll(cards=>cards.map(card=>{const typeLabel=card.querySelector('.stratagem-type'),head=card.querySelector('.stratagem-head'),cp=card.querySelector('.cp'),rail=getComputedStyle(card,'::before');return{id:card.dataset.ruleId,type:card.dataset.stratagemType,turn:card.dataset.turn,classes:[...card.classList],color:getComputedStyle(card).getPropertyValue('--strat-color').trim(),turnLabel:getComputedStyle(card,'::after').content.replace(/^"|"$/g,''),typeLabelCount:card.querySelectorAll('.stratagem-type').length,typeLabel:typeLabel?.textContent.trim()||'',typeLabelVisible:Boolean(typeLabel?.getClientRects().length),rendered:Boolean(card.getClientRects().length),railDisplay:rail.display,railWidth:parseFloat(rail.width),headerBorder:getComputedStyle(head).borderBottomColor,cpBorder:getComputedStyle(cp).borderColor};}));
    for(const fixture of [{label:'Death Guard',book:'death-guard'},{label:'Adeptus Mechanicus',book:'adeptus-mechanicus'}]){
      ...
      const staticTypes=await inspectTypes('.stratagem'),compatibleTrigger=page.locator('.related-rules-trigger').first();assert.equal(await compatibleTrigger.count(),1,`${fixture.label} desktop Compatible Stratagems trigger missing`);await compatibleTrigger.click();await page.waitForFunction(()=>document.querySelectorAll('.full-related-content .stratagem[data-stratagem-type]').length>=3);const compatibleTypes=await inspectTypes('.full-related-content .stratagem'),allTypes=[...staticTypes,...compatibleTypes],requireUnverified=fixture.book==='adeptus-mechanicus';desktopTypePalettes[fixture.book]=assertTypePalette(allTypes,`${fixture.label} desktop and Compatible Stratagems`,true,requireUnverified);if(requireUnverified){const defect=allTypes.filter(card=>card.id==='stratagem-defect-scrutiny');assert.equal(defect.length,2,'Mechanicus Defect Scrutiny must exist in desktop and Compatible Rules');assert.ok(defect.every(card=>card.type==='unknown'&&card.typeLabel==='Type unverified'),'Mechanicus Defect Scrutiny must remain visibly unverified');}await page.locator('.related-rules-close').click();
  ...
  try{
    ...
    for(const fixture of [{label:'Death Guard',book:'death-guard',staticRoute:'army-rules.html',unit:'mortarion.html',popupRoute:'army-rules.html',popupRelated:false},{label:'Adeptus Mechanicus',book:'adeptus-mechanicus',staticRoute:'cohort-acquisitus.html',unit:'skitarii-marshal.html',popupRoute:'skitarii-marshal.html',popupRelated:true}]){
      await page.setViewportSize({width:390,height:844});
      await page.goto(`${origin}/books/${fixture.book}/mobile/${fixture.staticRoute}`);await page.waitForFunction(()=>document.querySelectorAll('main .stratagem[data-turn][data-stratagem-type]').length>=3);const staticTypes=await inspectTypes('main .stratagem'),requireUnverified=fixture.book==='adeptus-mechanicus';assertTypePalette(staticTypes,`${fixture.label} static Phone Stratagems`,false,requireUnverified);
      await page.goto(`${origin}/books/${fixture.book}/mobile/${fixture.unit}`);const related=page.locator('#relatedRules');await related.scrollIntoViewIfNeeded();await page.waitForFunction(()=>document.querySelectorAll('#relatedRulesContent .stratagem[data-turn][data-stratagem-type]').length>=3);const compatibleTypes=await inspectTypes('#relatedRulesContent .stratagem');phoneTypePalettes[fixture.book]=assertTypePalette(compatibleTypes,`${fixture.label} Compatible Phone Stratagems`,true,requireUnverified);if(requireUnverified){const defect=[...staticTypes,...compatibleTypes].filter(card=>card.id==='stratagem-defect-scrutiny');assert.equal(defect.length,2,'Mechanicus Defect Scrutiny must exist in static and Compatible Phone views');assert.ok(defect.every(card=>card.type==='unknown'&&card.typeLabel==='Type unverified'),'Mechanicus Phone Defect Scrutiny must remain visibly unverified');}for(const type of canonicalStratagemTypes)assert.equal(phoneTypePalettes[fixture.book][type],desktopTypePalettes[fixture.book][type],`${fixture.label} ${type} differs between desktop and Phone`);
  ...
  const genericTypeContext=await browser.newContext({serviceWorkers:'block',viewport:{width:1440,height:900}});
  try{
    const {page,errors}=await observedPage(genericTypeContext),evidence={};
    const expectedColors={'battle-tactic':'#247090','strategic-ploy':'#507f72',wargear:'#a77b32','epic-deed':'#a83338',core:'#756f61',unknown:'#756f61'};
    const inspectTypes=selector=>page.locator(selector).evaluateAll(cards=>cards.map(card=>{const typeLabel=card.querySelector('.stratagem-type'),head=card.querySelector('.stratagem-head'),cp=card.querySelector('.cp'),rail=getComputedStyle(card,'::before');return{id:card.dataset.ruleId,type:card.dataset.stratagemType,turn:card.dataset.turn,classes:[...card.classList],color:getComputedStyle(card).getPropertyValue('--strat-color').trim(),turnLabel:getComputedStyle(card,'::after').content.replace(/^"|"$/g,''),typeLabelCount:card.querySelectorAll('.stratagem-type').length,typeLabel:typeLabel?.textContent.trim()||'',typeLabelVisible:Boolean(typeLabel?.getClientRects().length),rendered:Boolean(card.getClientRects().length),railDisplay:rail.display,railWidth:parseFloat(rail.width),headerBorder:getComputedStyle(head).borderBottomColor,cpBorder:getComputedStyle(cp).borderColor};}));
    const assertSurface=(cards,label)=>{assert.ok(cards.length,`${label} has no Stratagem cards`);assertTypePalette(cards,label,false,true);for(const card of cards)assert.equal(card.color,expectedColors[card.type],`${label} primary color disagrees with canonical type: ${JSON.stringify(card)}`);return cards.length;};
    const fixtures=[
      {book:'tyranids',label:'Tyranids',detachment:'invasion-fleet.html',unit:'hive-tyrant.html',unitId:'unit-hive-tyrant',types:['battle-tactic','strategic-ploy','epic-deed','core','unknown'],roster:{id:'tyr-visible-types',roster:{faction:'Tyranids',detachment:'Invasion Fleet',detachments:[{label:'Invasion Fleet'}],units:[{id:'tyr-visible-1',name:'Hive Tyrant',quantity:1,points:205}]}}},
      {book:'tau-empire',label:"T'au Empire",detachment:'kauyon.html',unit:'cadre-fireblade.html',unitId:'unit-cadre-fireblade',types:['battle-tactic','strategic-ploy','wargear','epic-deed','core','unknown'],roster:{id:'tau-visible-types',roster:{faction:"T'au Empire",detachment:'Kauyon',detachments:[{label:'Kauyon'}],units:[{id:'tau-visible-1',name:'Cadre Fireblade',quantity:1,points:50}]}}}
    ];
    for(const fixture of fixtures){
      evidence[fixture.book]={};
      await page.goto(`${origin}/books/${fixture.book}/reader.html`);
      await page.waitForFunction(()=>[...document.querySelectorAll('.stratagem')].every(card=>card.dataset.stratagemType&&card.dataset.turn&&card.querySelectorAll('.stratagem-type').length===1));
      const desktop=await inspectTypes('.stratagem');evidence[fixture.book].desktop=assertSurface(desktop,`${fixture.label} desktop`);
      for(const viewport of [{width:1440,height:900},{width:1194,height:834},{width:834,height:1194}]){await page.setViewportSize(viewport);assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=document.documentElement.clientWidth),true,`${fixture.label} desktop ${viewport.width}x${viewport.height} overflows horizontally`);assert.equal(await page.locator('.stratagem:visible .stratagem-type').count(),await page.locator('.stratagem:visible').count(),`${fixture.label} desktop ${viewport.width}x${viewport.height} visible label inventory mismatch`);}
      const desktopTrigger=page.locator(`#${fixture.unitId} .related-rules-trigger`);await desktopTrigger.click();await page.waitForFunction(()=>document.querySelectorAll('.full-related-content .stratagem').length>0&&[...document.querySelectorAll('.full-related-content .stratagem')].every(card=>card.querySelectorAll('.stratagem-type').length===1));const desktopDynamic=await inspectTypes('.full-related-content .stratagem');evidence[fixture.book].desktopDynamic=assertSurface(desktopDynamic,`${fixture.label} desktop Compatible Rules`);await page.locator('.related-rules-close').click();
      await page.setViewportSize({width:390,height:844});await page.goto(`${origin}/books/${fixture.book}/mobile/${fixture.detachment}`);await page.waitForFunction(()=>document.querySelectorAll('main .stratagem').length>0&&[...document.querySelectorAll('main .stratagem')].every(card=>card.dataset.stratagemType&&card.dataset.turn&&card.querySelectorAll('.stratagem-type').length===1));const phone=await inspectTypes('main .stratagem');evidence[fixture.book].phone=assertSurface(phone,`${fixture.label} Phone`);for(const viewport of [{width:390,height:844},{width:320,height:844}]){await page.setViewportSize(viewport);assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=document.documentElement.clientWidth),true,`${fixture.label} Phone ${viewport.width}x${viewport.height} overflows horizontally`);assert.equal(await page.locator('main .stratagem:visible .stratagem-type').count(),await page.locator('main .stratagem:visible').count(),`${fixture.label} Phone ${viewport.width}x${viewport.height} visible label inventory mismatch`);}
      await page.goto(`${origin}/books/${fixture.book}/mobile/${fixture.unit}`);await page.locator('#relatedRules').scrollIntoViewIfNeeded();await page.waitForFunction(()=>document.querySelectorAll('#relatedRulesContent .stratagem').length>0&&[...document.querySelectorAll('#relatedRulesContent .stratagem')].every(card=>card.querySelectorAll('.stratagem-type').length===1));const phoneDynamic=await inspectTypes('#relatedRulesContent .stratagem');evidence[fixture.book].phoneDynamic=assertSurface(phoneDynamic,`${fixture.label} Phone Related Rules`);
      await page.goto(`${origin}/index.html?visible-types=1`);await page.evaluate(record=>localStorage.setItem('wh40k-rosters-v1',JSON.stringify([record])),fixture.roster);
      await page.setViewportSize({width:1440,height:900});await page.goto(`${origin}/books/${fixture.book}/reader.html?roster=${fixture.roster.id}#${fixture.unitId}`);const rosterTrigger=page.locator(`#${fixture.unitId} .related-rules-trigger`);await rosterTrigger.click();await page.waitForFunction(()=>document.querySelectorAll('.full-related-content .stratagem').length>0&&[...document.querySelectorAll('.full-related-content .stratagem')].every(card=>card.querySelectorAll('.stratagem-type').length===1));const rosterDesktop=await inspectTypes('.full-related-content .stratagem');evidence[fixture.book].rosterDesktop=assertSurface(rosterDesktop,`${fixture.label} roster desktop`);
      await page.setViewportSize({width:390,height:844});await page.goto(`${origin}/books/${fixture.book}/mobile/${fixture.unit}?roster=${fixture.roster.id}`);await page.locator('#relatedRules').scrollIntoViewIfNeeded();await page.waitForFunction(()=>document.querySelectorAll('#relatedRulesContent .stratagem').length>0&&[...document.querySelectorAll('#relatedRulesContent .stratagem')].every(card=>card.querySelectorAll('.stratagem-type').length===1));const rosterPhone=await inspectTypes('#relatedRulesContent .stratagem');evidence[fixture.book].rosterPhone=assertSurface(rosterPhone,`${fixture.label} roster Phone`);
      const all=[...desktop,...desktopDynamic,...phone,...phoneDynamic,...rosterDesktop,...rosterPhone];for(const type of fixture.types)assert.ok(all.some(card=>card.type===type),`${fixture.label} lacks browser evidence for ${type}`);for(const type of fixture.types)assert.equal(new Set(all.filter(card=>card.type===type).map(card=>card.color)).size,1,`${fixture.label} ${type} color differs across rendering paths`);evidence[fixture.book].duplicates=all.filter(card=>card.typeLabelCount!==1).length;evidence[fixture.book].unknown=[...new Set(all.filter(card=>card.type==='unknown').map(card=>card.id))].sort();
    }
    assert.deepEqual(errors,[]);
    console.log(`PASS Tyranids and T'au visible Stratagem type contract ${JSON.stringify(evidence)}`);
  }finally{await genericTypeContext.close();}
  ...
  try{
    ...
    const tyrDialog=page.locator('#termDialog');await tyrDialog.waitFor({state:'visible'});const tyrCard=page.locator('#termPopupStack .mobile-popup-card[data-popup-term="core-anti"]');await tyrCard.waitFor({state:'visible'});assert.match(await tyrCard.textContent(),/ANTI/i);const tyrClose=page.locator('[data-popup-close="0"]');assert.equal(await tyrClose.count(),1);await tyrClose.click();await tyrDialog.waitFor({state:'hidden'});assert.equal(await tyrPhoneToken.evaluate(node=>node===document.activeElement),true,'Tyranids Phone ANTI-MONSTER token must regain focus');
    const tyrFocusToken=page.locator(tyrSelector).getByRole('button',{name:'ANTI-VEHICLE 5+',exact:true});await tyrFocusToken.click();await tyrDialog.waitFor({state:'visible'});const tyrSecondCard=page.locator('#termPopupStack .mobile-popup-card[data-popup-term="core-anti"]');await tyrSecondCard.waitFor({state:'visible'});assert.match(await tyrSecondCard.textContent(),/ANTI/i);const tyrSecondClose=page.locator('[data-popup-close="0"]');assert.equal(await tyrSecondClose.count(),1);await tyrSecondClose.click();await tyrDialog.waitFor({state:'hidden'});assert.equal(await tyrFocusToken.evaluate(node=>node===document.activeElement),true,'Tyranids Phone ANTI-VEHICLE token must regain focus after pointer activation');
  ...
  try{
    ...
    const expectDeathGuardPhoneRosterRejected=async label=>{
      await page.waitForURL('**/roster-guides/index.html*');
      assert.equal(new URL(page.url()).pathname,'/roster-guides/index.html',`${label}: canonical Roster Guides destination`);
      for(const selector of ['#relatedRules','#relatedRulesContent','.unit-card','.related-rules-trigger','[data-roster-active]'])assert.equal(await page.locator(selector).count(),0,`${label}: ${selector} must be absent after replacement`);
    };
    await expectDeathGuardPhoneRosterRejected('wrong-faction Death Guard Phone roster');
    await page.goto(`${origin}/index.html?dg-phone-roster-control=1`);
    await page.goto(`${origin}/books/death-guard/mobile/helbrute.html?view=mobile&roster=missing-dg-history`);
    await expectDeathGuardPhoneRosterRejected('Death Guard Phone replacement-history fixture');
    await page.goBack();
    await page.waitForURL(url=>url.pathname.endsWith('/index.html')&&url.search==='?dg-phone-roster-control=1');
    ...
    await page.goto(`${origin}/books/death-guard/mobile/helbrute.html?view=mobile&roster=missing-roster`);
    await expectDeathGuardPhoneRosterRejected('missing Death Guard Phone roster');
    await page.evaluate(()=>localStorage.setItem('wh40k-rosters-v1',JSON.stringify([{id:'dg-multiple-detachments',roster:{faction:'Death Guard',detachments:[{label:'Contagion Engines'},{label:'Virulent Vectorium'}],units:[{id:'dg-multi-owner',name:'Helbrute',quantity:1,points:110}]}}])));
    await page.goto(`${origin}/books/death-guard/mobile/helbrute.html?view=mobile&roster=dg-multiple-detachments`);
    await expectDeathGuardPhoneRosterRejected('multiple-Detachment Death Guard Phone roster');
    await page.evaluate(()=>localStorage.setItem('wh40k-rosters-v1','{broken'));
    ...
    await expectDeathGuardPhoneRosterRejected('corrupt Death Guard Phone roster');
    await page.evaluate(()=>localStorage.removeItem('death-guard-detachment-filter'));
  ...
  try{
    ...
    const expectTyranidsRoundtrip=async faction=>{
      await page.goto(origin+'/roster-guides/index.html');
      const record=await create(faction,{detachment:'Invasion Fleet',units:'Char1: 1x Hive Tyrant (205 pts)'});
      assert.equal(record.roster.faction,'Tyranids',faction+': Roster Guides canonical faction');assert.ok(record.sourceText.includes('FACTION KEYWORD: '+faction),faction+': raw sourceText preserved');
      await page.locator('#open-guide').click();await page.waitForURL('**/books/tyranids/reader.html**');assert.equal(new URL(page.url()).searchParams.get('roster'),record.id,faction+': desktop roster query');assert.deepEqual(await page.evaluate(()=>window.TYRANIDS_ROSTER_GUIDE?.detachmentIds),['invasion-fleet'],faction+': desktop Detachment');assert.equal(await page.locator('#unit-hive-tyrant .related-rules-trigger').count(),1,faction+': desktop Compatible Rules');
    ...
    await page.goto(`${origin}/roster-guides/index.html`);
    for(const faction of ['Tyranids','Xenos - Tyranids','Xenos – Tyranids','Xenos — Tyranids'])await expectTyranidsRoundtrip(faction);
    await page.goto(`${origin}/roster-guides/index.html`);
  ...
  try{
    ...
    const expectTauPhoneRosterRejected=async id=>{await page.goto(`${origin}/books/tau-empire/mobile/cadre-fireblade.html?roster=${id}`);await page.waitForURL('**/roster-guides/index.html');assert.equal(await page.locator('#relatedRules').count(),0,`${id} T'au Phone roster must not retain Related Rules`);assert.equal(await page.locator('.unit-card').count(),0,`${id} T'au Phone roster must not retain datasheets`);assert.notEqual(await page.locator('html').getAttribute('data-roster-active'),'true',`${id} T'au Phone roster must not remain active`);};
    for(const id of ['tau-wrong-faction','tau-imperium-prefix','tau-chaos-prefix']){await expectTauPhoneRosterRejected(id);await page.goto(`${origin}/books/tau-empire/reader.html?roster=${id}#unit-cadre-fireblade`);await page.waitForURL('**/roster-guides/index.html');assert.equal(await page.locator('.related-rules-trigger').count(),0,`${id} T'au desktop roster must fail closed`);}
    await expectTauPhoneRosterRejected('missing-tau');await page.goto(`${origin}/index.html`);await page.evaluate(()=>localStorage.setItem('wh40k-rosters-v1','{broken'));await expectTauPhoneRosterRejected('broken-tau');await page.evaluate(()=>localStorage.removeItem('wh40k-rosters-v1'));
    assert.deepEqual(errors,[]);
  ...
  try{
    ...
    const shell=await page.evaluate(async()=>({controller:typeof window.TYRPhonePopups==='function',css:Boolean(await caches.match(new URL('./mobile.css?v=2',location.href).href)),mobile:Boolean(await caches.match(new URL('./mobile.js?v=9',location.href).href)),popup:Boolean(await caches.match(new URL('./phone-popup-controller.js?v=1',location.href).href))}));
