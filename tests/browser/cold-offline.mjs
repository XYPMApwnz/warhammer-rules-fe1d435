...
try{
  ...
  try{
    ...
    await page.goto(popupRoute);await page.waitForSelector('#relatedRulesContent .related-empty, #relatedRulesContent .related-detachment');const touchRoot=page.locator('main [data-term="core-support"]').first(),touchBox=await touchRoot.boundingBox();assert.ok(touchBox);await touchRoot.dispatchEvent('pointerdown',{pointerType:'touch',pointerId:71,isPrimary:true,clientX:touchBox.x+5,clientY:touchBox.y+5});await touchRoot.dispatchEvent('pointermove',{pointerType:'touch',pointerId:71,isPrimary:true,clientX:touchBox.x+25,clientY:touchBox.y+25});await touchRoot.dispatchEvent('pointerup',{pointerType:'touch',pointerId:71,isPrimary:true,clientX:touchBox.x+25,clientY:touchBox.y+25});assert.equal(await page.locator('#termDialog').evaluate(node=>node.open),false);await page.touchscreen.tap(touchBox.x+5,touchBox.y+5);assert.equal(await page.locator('#termPopupStack .mobile-popup-card').count(),1);await page.mouse.click(1,1);
