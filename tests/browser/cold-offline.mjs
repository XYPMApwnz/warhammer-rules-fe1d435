...
try{
  ...
  try{
    ...
    for(const fixture of [{label:'Death Guard',book:'death-guard'},{label:'Adeptus Mechanicus',book:'adeptus-mechanicus'}]){
      ...
      await page.waitForFunction(()=>[...document.querySelectorAll('.stratagem-grid')].some(grid=>grid.getClientRects().length&&grid.querySelectorAll(':scope > .stratagem').length>=2));
  ...
  const responsivePhoneContext=await browser.newContext({serviceWorkers:'block',viewport:{width:390,height:844},hasTouch:true,isMobile:true});
  try{
    const {page,errors}=await observedPage(responsivePhoneContext);
    const inspectTurns=selector=>page.locator(selector).evaluateAll(cards=>cards.map(card=>({turn:card.dataset.turn,classes:[...card.classList],color:getComputedStyle(card).getPropertyValue('--strat-color').trim(),label:getComputedStyle(card,'::after').content.replace(/^"|"$/g,'')})));
    const assertTurns=(cards,label)=>{for(const [turn,className] of [['ANY TURN','turn-any'],['YOUR TURN','turn-yours'],['THEIR TURN','turn-their']]){const card=cards.find(item=>item.turn===turn);assert.ok(card,`${label} lacks ${turn}`);assert.ok(card.classes.includes(className),`${label} ${turn} lacks ${className}`);assert.equal(card.label,turn,`${label} ${turn} badge mismatch`);}assert.equal(new Set(cards.filter(card=>['ANY TURN','YOUR TURN','THEIR TURN'].includes(card.turn)).map(card=>card.color)).size,3,`${label} turn colors must be distinct`);};
    ...
    for(const fixture of [{label:'Death Guard',book:'death-guard',staticRoute:'army-rules.html',unit:'mortarion.html',popupRoute:'army-rules.html',popupRelated:false},{label:'Adeptus Mechanicus',book:'adeptus-mechanicus',staticRoute:'data-psalm-conclave.html',unit:'skitarii-marshal.html',popupRoute:'skitarii-marshal.html',popupRelated:true}]){
      await page.setViewportSize({width:390,height:844});
      await page.goto(`${origin}/books/${fixture.book}/mobile/${fixture.staticRoute}`);await page.waitForFunction(()=>document.querySelectorAll('main .stratagem[data-turn]').length>=3);assertTurns(await inspectTurns('main .stratagem'),`${fixture.label} static Phone Stratagems`);
      await page.goto(`${origin}/books/${fixture.book}/mobile/${fixture.unit}`);const related=page.locator('#relatedRules');await related.scrollIntoViewIfNeeded();await page.waitForFunction(()=>document.querySelectorAll('#relatedRulesContent .stratagem[data-turn]').length>=3);assertTurns(await inspectTurns('#relatedRulesContent .stratagem'),`${fixture.label} Compatible Phone Stratagems`);
    ...
    assert.deepEqual(errors,[]);
    console.log('PASS Death Guard and Adeptus Mechanicus Phone turn colors and content-sized popup geometry');
  ...
  try{
    ...
    const warmDgState=await page.evaluate(async()=>({
      ...
      cachedCss:Boolean(await caches.match(new URL('./mobile.css?v=10',location.href).href)),
      cachedMobile:Boolean(await caches.match(new URL('./mobile.js?v=27',location.href).href)),
