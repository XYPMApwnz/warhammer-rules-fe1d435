...
const rawRecord=(id,{needle=true,attached=true,suffixed=false}={})=>({id,sourceText:needle?(suffixed?sourceTextSuffixed:sourceText):sourceTextNoNeedle,attachments:attached?{'parsed-unit-2':['parsed-unit-1']}: {}});
const canonicalRecord=(id,{needle=true,attached=true}={})=>createRosterFixture({catalog,pointsCatalog,id,detachmentId:'detachment-champions-of-contagion',units:[
  {datasheetId:'unit-plague-surgeon',instanceId:'parsed-unit-1',quantity:1,...(needle?{enhancementId:needleId}:{})},
...
try{
  ...
  const attached=await open(rawRecord('needle-attached'),'parsed-unit-2','unit-plague-marines');
  ...
  const suffixed=await open(rawRecord('needle-suffixed',{suffixed:true}),'parsed-unit-2','unit-plague-marines');
  ...
  const other=await open(canonicalRecord('needle-isolation'),'parsed-unit-3','unit-plague-marines');
  ...
  const noNeedle=await open(canonicalRecord('tainted-no-needle',{needle:false}),'parsed-unit-2','unit-plague-marines');
