...
for(const book of books){
  ...
  const stale=attachmentsApi.sanitize({units,attachments:{[bodyguardId]:[`${book.slug}-outsider`],missing:[leaderBId]}});assert.deepEqual(Object.keys(stale.attachments),[],`${book.name}: stale mapping`);assert.deepEqual([...stale.rejected].map(({bodyguardInstanceId,leaderInstanceId,reason})=>({bodyguardInstanceId,leaderInstanceId,reason})),[
    {bodyguardInstanceId:bodyguardId,leaderInstanceId:`${book.slug}-outsider`,reason:'pair-not-canonical'},
    {bodyguardInstanceId:'missing',leaderInstanceId:leaderBId,reason:'missing-bodyguard'}
  ],`${book.name}: exact stale rejection identities`);
...
try{
  ...
  try{const page=await context.newPage(),errors=[];page.on('pageerror',error=>errors.push(error.message));
    for(const book of books){const fixture=fixtures.get(book.slug),{record,bodyguardId,leaderAId,leaderBId,sample}=fixture;console.log(`CHECK ${book.name}`);
      await page.goto(`${origin}/books/${book.slug}/reader.html?view=mobile&roster=${record.id}#${sample.bodyguard.unitId}`);await waitRoster(page,bodyguardId);assert.doesNotMatch((await page.locator('.roster-game-summary').allInnerTexts()).join(''),/(?:Leader|Leading):/,`${book.name}: potential became current before action`);
      ...
      await page.goto(`${origin}/books/${book.slug}/reader.html?view=mobile&roster=${record.id}&rosterInstance=${leaderAId}#${sample.leader.unitId}`);await waitRoster(page,leaderAId);let leader=await page.evaluate(()=>{const card=document.querySelector('.unit-card.roster-game-view'),ids=[...document.querySelectorAll('[id]')].map(node=>node.id);return{id:card.dataset.rosterGameInstance,summary:card.querySelector('.roster-game-summary')?.innerText||'',cards:document.querySelectorAll('.unit-card.roster-game-view').length,duplicates:[...new Set(ids.filter((id,index)=>ids.indexOf(id)!==index))]};});assert.deepEqual({id:leader.id,cards:leader.cards,duplicates:leader.duplicates},{id:leaderAId,cards:1,duplicates:[]},`${book.name}: exact leader A card`);assert.doesNotMatch(leader.summary,/Leading:/,`${book.name}: duplicate leader A leakage`);await page.goto(`${origin}/books/${book.slug}/reader.html?view=mobile&roster=${record.id}&rosterInstance=${leaderBId}#${sample.leader.unitId}`);await waitRoster(page,leaderBId);leader=await page.evaluate(()=>{const card=document.querySelector('.unit-card.roster-game-view'),ids=[...document.querySelectorAll('[id]')].map(node=>node.id);return{id:card.dataset.rosterGameInstance,summary:card.querySelector('.roster-game-summary')?.innerText||'',cards:document.querySelectorAll('.unit-card.roster-game-view').length,duplicates:[...new Set(ids.filter((id,index)=>ids.indexOf(id)!==index))]};});assert.deepEqual({id:leader.id,cards:leader.cards,duplicates:leader.duplicates},{id:leaderBId,cards:1,duplicates:[]},`${book.name}: exact leader B card`);assert.match(leader.summary,new RegExp(`Leading: ${sample.bodyguard.title.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}`),`${book.name}: reverse Leading label`);
      ...
      await page.goto(`${origin}/books/${book.slug}/reader.html?view=mobile&roster=${record.id}#${sample.bodyguard.unitId}`);await waitRoster(page,bodyguardId);assert.doesNotMatch((await page.locator('.roster-game-summary').allInnerTexts()).join(''),/(?:Leader|Leading):/,`${book.name}: detach retained factual label`);
    }
