...
const profileAnchorContract=async(page,name)=>{
  ...
  await page.locator(`[data-journey-target="${profile.id}"]`).click();await waitForHash(page,profile.id);await page.waitForFunction(()=>document.querySelector('.logical-destination-highlight.destination-highlight'));
  const aligned=await page.evaluate(id=>{
    ...
    const members=Array.isArray(resolved.highlightTarget)?resolved.highlightTarget:[resolved.highlightTarget],statlines=[...unit.querySelectorAll('.statline')],bases=[...unit.querySelectorAll('.profile-base')],highlighted=[...document.querySelectorAll('.destination-highlight')],range=document.querySelector('.logical-destination-highlight.destination-highlight'),rangeBox=range.getBoundingClientRect(),memberBoxes=members.map(member=>member.getBoundingClientRect());
    const top=anchor.getBoundingClientRect().top,gap=window.DG_APP.navigation.trackingGap;
    return{visibleBelowNav:top>=nav.bottom-1&&top<=nav.bottom+gap+4,anchorBeforeSection:Boolean(anchor.compareDocumentPosition(section)&Node.DOCUMENT_POSITION_FOLLOWING),statlines:statlines.length,members:members.length,memberHighlights:members.filter(member=>member.classList.contains('destination-highlight')).length,visualRegions:highlighted.length,rangeTop:Math.abs(rangeBox.top-Math.min(...memberBoxes.map(box=>box.top)))<=1,rangeBottom:Math.abs(rangeBox.bottom-Math.max(...memberBoxes.map(box=>box.bottom)))<=1,statlinesIncluded:statlines.every(node=>rangeBox.top<=node.getBoundingClientRect().top+1&&rangeBox.bottom>=node.getBoundingClientRect().bottom-1),basesIncluded:bases.every(node=>rangeBox.top<=node.getBoundingClientRect().top+1&&rangeBox.bottom>=node.getBoundingClientRect().bottom-1),sectionIncluded:rangeBox.bottom>=section.getBoundingClientRect().bottom-1,animationInstances:range.getAnimations().length,lifecycleTargets:window.DG_APP.navigation.highlighter.current.length,lifecycleTimer:Boolean(window.DG_APP.navigation.highlighter.timer)};
  },profile.id);
  ...
  assert.equal(aligned.memberHighlights,0,`${name}: logical Profile members animate independently`);
  assert.equal(aligned.visualRegions,1,`${name}: logical Profile has duplicate visual highlight regions`);
  assert.equal(aligned.rangeTop,true,`${name}: Profile highlight does not start at the logical statline boundary`);
  assert.equal(aligned.rangeBottom,true,`${name}: Profile highlight does not end at Profile/Weapons content`);
  assert.equal(aligned.statlinesIncluded,true,`${name}: Profile feedback excludes the unit statline`);
  assert.equal(aligned.basesIncluded,true,`${name}: Profile feedback excludes Base`);
  assert.equal(aligned.sectionIncluded,true,`${name}: Profile feedback excludes Profile/Weapons content`);
  assert.equal(aligned.animationInstances,1,`${name}: logical Profile runs duplicate animations`);
  assert.equal(aligned.lifecycleTargets,1,`${name}: Highlighter has more than one visual target`);
