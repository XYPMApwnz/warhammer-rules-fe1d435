...
assert.equal(rows.length,1084);
assert.equal(rows.filter(rule=>rule.state==='match').length,977);
...
assert.equal(enhancements.length,206);
...
assert.equal(parity.officialOwnerAssociations,enhancements.filter(rule=>!upgradeIds.includes(rule.ruleId)).length);
