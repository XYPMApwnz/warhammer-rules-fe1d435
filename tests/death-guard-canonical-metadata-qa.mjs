...
const readerShell=read('books/death-guard/reader.html');
const targetSource=read('books/death-guard/scripts/target-data.js');
const reader=JSON.parse(targetSource.slice(targetSource.indexOf('Object.freeze(')+14,targetSource.lastIndexOf(');'))).html;
...
for(const match of readerShell.matchAll(/<button\b([^>]*)>([\s\S]*?)<\/button>/g))navigationButtons.push({attrs:match[1],label:strip(match[2])});
...
const currentNavigation=navigationButtons.filter(button=>attr(button.attrs,'data-nav-target')&&attr(button.attrs,'class').split(/\s+/).includes('toc-label')).map((button,order)=>({order,targetId:attr(button.attrs,'data-nav-target'),label:button.label.trim()}));
const metadataNavigation=metadata.navigation.nodes.map(({order,targetId,label})=>({order,targetId,label})).sort((left,right)=>left.order-right.order);
...
check(metadata.navigation.nodes.find(node=>node.targetId==='pact-of-decay-datasheets')?.parentId==='datasheets','Pact of Decay navigation ownership');

const owners=Object.fromEntries(metadata.sectionOwnership.records.map(record=>[record.id,record.parentId]));
equal(owners,{'mortarion-ability-supreme-commander':'unit-mortarion','miasmic-malignifier-ability-fortification-setup':'unit-miasmic-malignifier','great-unclean-one-ability-reverberating-summons':'unit-great-unclean-one','pact-of-decay-datasheets':'datasheets'},'section ownership contract');
