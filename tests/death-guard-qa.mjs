...
check('landscape tablet stratagem cards use two columns with a phone/portrait fallback',contentCss.includes('.detachment-part[id$="-stratagems"] > .detachment-content,')&&contentCss.includes('.detachment-part[id$="-stratagems"] > .detachment-grid { grid-template-columns: repeat(2, minmax(0, 1fr))')&&/@media\s*\(max-width:\s*900px\)[\s\S]*?grid-template-columns:\s*1fr/.test(contentCss));
