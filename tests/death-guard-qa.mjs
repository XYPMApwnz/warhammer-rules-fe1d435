...
check('no-query All Detachments behavior remains while an empty roster query fails closed',rosterFilterRuntime.includes('if (!rosterMode) return;')&&rosterFilterRuntime.includes('if (!rosterId) { location.replace("../../roster-guides/index.html"); return; }')&&mobileTyphus.includes('<option value="all">All detachments</option>')&&appSource.includes("[['all','All Detachments']]")&&mobileRuntime.indexOf('if (rosterMode)')<mobileRuntime.indexOf('option !== matchingOption'));
...
check('book scripts and styles use the current release token',[...cssFiles.filter(file=>!['styles/tokens.css','styles/layout.css','styles/content.css','styles/navigation.css','styles/popups.css'].includes(file)),...files.filter(file=>!['scripts/roster-filter.js','scripts/navigation-controller.js','scripts/popup-controller.js','scripts/full-entry-controller.js','scripts/journey-controller.js','scripts/ui-controllers.js','scripts/app.js'].includes(file))].every(file=>html.includes('./'+file+'?v=9'))&&html.includes('./styles/tokens.css?v=11')&&html.includes('./styles/layout.css?v=11')&&html.includes('./styles/content.css?v=40')&&html.includes('./styles/navigation.css?v=12')&&html.includes('./styles/popups.css?v=18')&&html.includes('./scripts/roster-filter.js?v=18')&&html.includes('./scripts/navigation-controller.js?v=16')&&html.includes('./scripts/popup-controller.js?v=25')&&html.includes('./scripts/full-entry-controller.js?v=9')&&html.includes('./scripts/journey-controller.js?v=13')&&html.includes('./scripts/ui-controllers.js?v=12')&&html.includes('./scripts/app.js?v=42'));
check('Death Guard roster routes replace invalid contexts before roster initialization',read('scripts/roster-filter.js').includes('if (!selected.size) { location.replace("../../roster-guides/index.html"); return; }')&&/location\.replace\('\.\.\/\.\.\/\.\.\/roster-guides\/index\.html'\);\s*return;/.test(read('mobile/mobile.js'))&&!/catch\s*\{[\s\S]{0,160}relatedRulesEnabled\s*=\s*false[\s\S]{0,160}relatedRules\?\.remove/.test(read('mobile/mobile.js')));
