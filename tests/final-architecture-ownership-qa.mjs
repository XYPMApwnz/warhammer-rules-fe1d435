...
const productionText=[read('service-worker.js'),read('books/shared/tools/build-army-book.mjs'),read('books/shared/tools/render-structured-effective-book.mjs'),read('books/death-guard/tools/presentation-hook.mjs'),...['death-guard','adeptus-mechanicus','tau-empire','emperors-children','tyranids','chaos-space-marines','space-marines','dark-angels','blood-angels'].flatMap(id=>[read(`books/${id}/reader.html`),read(`books/${id}/index.html`)])].join('\n');
