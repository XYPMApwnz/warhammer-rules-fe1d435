...
assert.equal(values.length,3234,'canonical entry count must remain stable');
...
assert.equal(crypto.createHash('sha256').update(registryIds.join('\n')).digest('hex'),'5beceffffc71d95199123391c25986ebc1b39f50fcf13c89d214213ccc86881e','canonical glossary identity set must remain stable');
...
assert.equal(searchable.length,3130,'only the 93 confirmed context-only entries may leave the catalogue');
...
assert.equal(weaponProfiles.length,1116,'weapon profile inventory must remain stable');
