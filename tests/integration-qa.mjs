...
for(const slug of generatedArmyBooks){
  ...
  check(`${slug} uses the shared DG interaction architecture`,html.includes('../death-guard/scripts/navigation-controller.js')&&html.includes('../death-guard/scripts/popup-controller.js')&&html.includes('../death-guard/scripts/journey-controller.js')&&app.includes('WHArmyBook.install'));
  check(`${slug} loads base tokens before faction overrides`,html.indexOf('../death-guard/styles/tokens.css?v=10')>=0&&html.indexOf('../death-guard/styles/tokens.css?v=10')<html.indexOf('./styles/tokens.css?v=1'));
  ...
  check(`${slug} exposes the forced Phone view contract`,config.dedicatedMobile?app.includes('"dedicatedMobile":true')&&mobile.includes('Desktop / iPad view')&&mobile.includes('data-view-switch'):app.includes('readerPath')&&mobile.includes('searchParams.set("view","mobile")')&&bookCss.includes('html[data-view="mobile"] .nav-menu')&&bookCss.includes('html[data-view="mobile"] .unit-card.ds-layout .ds-main-grid'));
...
check('Mega Glossary return UI is shared, versioned and precached',read('glossary/index.html').includes('id="libraryBack"')&&read('glossary/index.html').includes('../glossary-return.js?v=2')&&read('glossary/index.html').includes('viewer.js?v=9')&&sw.includes('"./glossary-return.js?v=2"')&&sw.includes('"./glossary/viewer.js?v=9"')&&sw.includes('"./glossary/viewer-progressive.css?v=2"'));
...
check('Every reader can restore its originating term popup',read('glossary-return.js').includes('isExactReturnTarget')&&read('glossary-return.js').includes("'automatic'")&&read('books/core-rules/reader/core-concepts.html').includes('../../../glossary-return.js?v=2')&&read('books/core-rules/reader/app.js').includes('triggerIndex')&&read('books/death-guard/mobile/mortarion.html').includes('../../../glossary-return.js?v=2')&&read('books/death-guard/mobile/mobile.js').includes('triggerIndex'));
...
check('Mechanicus Related Rules use explicit eligibility in both views',read('books/adeptus-mechanicus/scripts/related-rules.js').includes("JSON.parse(card.dataset.eligibility||'')")&&read('books/adeptus-mechanicus/mobile/related-rules.inc').includes('data-eligibility='));
...
check('Army books use one shared unit eligibility matcher',read('books/death-guard/reader.html').includes('shared/related-rules-matcher.js?v=1')&&read('books/death-guard/mobile/mortarion.html').includes('shared/related-rules-matcher.js?v=1')&&read('books/adeptus-mechanicus/reader.html').includes('shared/related-rules-matcher.js?v=1'));
