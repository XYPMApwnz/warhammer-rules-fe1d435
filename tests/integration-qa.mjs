...
const books={
  'death-guard':{version:'9',reader:'reader.html',versions:{'styles/tokens.css':'11','styles/layout.css':'11','styles/navigation.css':'12','styles/content.css':'40','styles/popups.css':'18','scripts/roster-filter.js':'17','scripts/navigation-controller.js':'16','scripts/popup-controller.js':'25','scripts/full-entry-controller.js':'9','scripts/journey-controller.js':'13','scripts/ui-controllers.js':'12','scripts/app.js':'42'},app:'scripts/app.js',usesPopupGlossary:true,files:['assets/icon-v4.svg','styles/tokens.css','styles/layout.css','styles/navigation.css','styles/content.css','styles/popups.css','scripts/roster-filter.js','scripts/navigation-controller.js','scripts/popup-controller.js','scripts/full-entry-controller.js','scripts/journey-controller.js','scripts/ui-controllers.js','scripts/app.js']},
...
for(const slug of generatedArmyBooks){
  ...
  check(`${slug} exposes Library, Glossary and Related Rules`,html.includes('href="../../index.html"')&&html.includes('href="../../glossary/index.html"')&&(config.compatibleRulesMatrix?app.includes('compatible-rules-runtime.mjs'):html.includes(`../shared/army-related-rules.js?v=${relatedRuntimeVersion}`)));
  check(`${slug} exposes the forced Phone view contract`,config.dedicatedMobile?app.includes("new URL('./mobile/'+route")&&mobile.includes('Desktop / iPad view')&&mobile.includes('data-view-switch')&&!bookCss.includes('html[data-view="mobile"]'):config.customApp?app.includes("phoneMode=params.get('view')==='mobile'")&&mobile.includes('searchParams.set("view","mobile")')&&bookCss.includes('html[data-view="mobile"] .nav-menu'):app.includes('readerPath')&&mobile.includes('searchParams.set("view","mobile")')&&bookCss.includes('html[data-view="mobile"] .nav-menu')&&bookCss.includes('html[data-view="mobile"] .unit-card.ds-layout .ds-main-grid'));
...
check('Roster Guides preserve the storage contract',rosterGuidesApp.includes("const STORAGE_KEY='wh40k-rosters-v1'")&&rosterGuidesApp.includes('function rosterId(text)')&&rosterGuidesApp.includes("'death guard':'../books/death-guard/reader.html'")&&rosterGuides.includes('app.js?v=11')&&sw.includes('./roster-guides/app.js?v=11'));
...
check('Roster Guides recognise the five supported factions and expose only real readers',rosterGuidesApp.includes("new Set(['death guard','adeptus mechanicus','tyranids','t au empire','emperor s children'])")&&rosterGuidesApp.includes("'death guard':'../books/death-guard/reader.html'")&&rosterGuidesApp.includes("'adeptus mechanicus':'../books/adeptus-mechanicus/index.html'")&&rosterGuidesApp.includes("'tyranids':'../books/tyranids/index.html'")&&rosterGuidesApp.includes("'t au empire':'../books/tau-empire/index.html'")&&rosterGuidesApp.includes("'emperor s children':'../books/emperors-children/index.html'"));
check('Roster Guides canonicalise faction aliases and validate explicit parents',rosterGuidesApp.includes("'tau empire':'t au empire'")&&rosterGuidesApp.includes("'t au empire':'xenos'")&&rosterGuidesApp.includes('[-–—]')&&rosterGuidesApp.includes('FACTION_PARENTS[key]===parent')&&rosterGuidesApp.includes('knownFaction(record.roster.faction)'));
...
check('Mechanicus records open the real responsive reader',rosterGuidesApp.includes("'adeptus mechanicus':'../books/adeptus-mechanicus/index.html'")&&read('books/adeptus-mechanicus/reader.html').includes('scripts/roster-filter.js?v=4'));
check("T'au records open the dedicated responsive reader",rosterGuidesApp.includes("'t au empire':'../books/tau-empire/index.html'")&&read('books/tau-empire/reader.html').includes('roster-parser.js?v=2')&&read('books/tau-empire/scripts/roster-filter.js').includes("localStorage.getItem('wh40k-rosters-v1')"));
...
const glossaryReturnSources=[read('books/core-rules/reader/app.js'),read('books/death-guard/mobile/mobile.js'),read('books/death-guard/scripts/app.js'),read('books/adeptus-mechanicus/scripts/app.js')];
const restoresAfterScroll=source=>{
  const readIndex=Math.max(source.indexOf('WHGlossaryReturn?.read'),source.indexOf('restorePopup(returnRecord)')),scrollIndex=source.indexOf('window.scrollTo',readIndex),nextFrameIndex=source.indexOf('requestAnimationFrame',scrollIndex+'window.scrollTo'.length),clearIndex=source.indexOf('WHGlossaryReturn.clear',nextFrameIndex);
  return readIndex>=0&&scrollIndex>readIndex&&nextFrameIndex>scrollIndex&&clearIndex>nextFrameIndex&&/(?:\.(?:restore|click)\(|showTerm\()/.test(source.slice(nextFrameIndex,clearIndex));
};
const popupIdReturnSources=glossaryReturnSources.filter(source=>source.includes('popupIds'));
const restoresSavedPopupIds=source=>{
  const readIndex=source.indexOf('WHGlossaryReturn?.read'),scrollIndex=source.indexOf('window.scrollTo',readIndex),nextFrameIndex=source.indexOf('requestAnimationFrame',scrollIndex+'window.scrollTo'.length),clearIndex=source.indexOf('WHGlossaryReturn.clear',nextFrameIndex),popupIdsIndex=source.indexOf('popupIds',readIndex);
  return popupIdsIndex>readIndex&&popupIdsIndex<clearIndex&&source.slice(nextFrameIndex,clearIndex).includes('popupIds');
};
...
check('Open popups lock background scrolling',read('books/core-rules/reader/styles.css').includes('html:has(dialog[open])')&&read('books/death-guard/styles/popups.css').includes('body:has(.popup-layer .term-popup)')&&read('books/adeptus-mechanicus/reader.html').includes('../death-guard/styles/popups.css?v=18')&&read('glossary/viewer-progressive.css').includes('body:has(dialog[open])'));
...
check('glossary return restores scroll before reopening popups',glossaryReturnSources.every(restoresAfterScroll)&&popupIdReturnSources.length>0&&popupIdReturnSources.every(restoresSavedPopupIds));
...
check('shared datasheet design is loaded and precached',['death-guard','adeptus-mechanicus'].every(slug=>bookHtml(slug).includes('href="../shared/datasheet-system.css?v=7"'))&&sw.includes('"./books/shared/datasheet-system.css?v=7"'));
...
check('shared datasheet layout is loaded and precached',['death-guard','adeptus-mechanicus'].every(slug=>bookHtml(slug).includes('src="../shared/datasheet-layout.js?v=3"'))&&sw.includes('"./books/shared/datasheet-layout.js?v=3"'));
check('shared long datasheet abilities continue at full width with original nodes',read('books/shared/datasheet-layout.js').includes("layout.continuation.className='ability-list ds-abilities-continuation'")&&read('books/shared/datasheet-layout.js').includes('layout.cards.slice(split).forEach(node=>layout.continuation.append(node))'));
...
const amPhoneContext={window:{},URL};vm.runInNewContext(read('books/adeptus-mechanicus/mobile/mobile.js'),amPhoneContext,{filename:'books/adeptus-mechanicus/mobile/mobile.js'});const amPhoneRoster=amPhoneContext.window.AMPhoneRoster,rosterParams=new URLSearchParams([['roster','mechanicus-roster'],['case','cross-view']]),desktopView=new URL(amPhoneRoster.viewSwitchHref('https://example.test/books/adeptus-mechanicus/reader.html?stale=1#stale',rosterParams.toString(),'#unit-skitarii-rangers')),phoneView=new URL(amPhoneRoster.viewSwitchHref('https://example.test/books/adeptus-mechanicus/mobile/skitarii-rangers.html',rosterParams.toString(),'#unit-skitarii-rangers')),plainView=new URL(amPhoneRoster.viewSwitchHref('https://example.test/books/adeptus-mechanicus/mobile/index.html','case=reference','#start'));
check('Mechanicus preserves roster and rule context across views',desktopView.pathname==='/books/adeptus-mechanicus/reader.html'&&phoneView.pathname==='/books/adeptus-mechanicus/mobile/skitarii-rangers.html'&&desktopView.searchParams.get('roster')==='mechanicus-roster'&&desktopView.searchParams.get('case')==='cross-view'&&desktopView.searchParams.getAll('roster').length===1&&desktopView.hash==='#unit-skitarii-rangers'&&phoneView.search===desktopView.search&&phoneView.hash===desktopView.hash&&!plainView.searchParams.has('roster')&&plainView.searchParams.get('case')==='reference'&&plainView.hash==='#start'&&read('books/adeptus-mechanicus/mobile/index.html').includes('data-view-switch'));
...
const dgPhonePage=read('books/death-guard/mobile/mortarion.html'),dgPhoneRuntime=read('books/death-guard/mobile/mobile.js'),dgPhonePopups=read('books/death-guard/mobile/phone-popup-controller.js');
check('Death Guard Phone uses canonical glossary with page-local fallback metadata',dgPhonePage.includes('data-term-summary=')&&dgPhonePage.includes('../../../glossary/generated/glossary.en.js?v=tyranids-1')&&(dgPhonePage.match(/glossary\/generated\/glossary\.en\.js/g)||[]).length===1&&dgPhoneRuntime.includes("WH40K_GLOSSARY.forBook('death-guard')")&&dgPhoneRuntime.includes("WHGlossaryAutolink?.configure('death-guard')")&&dgPhonePopups.includes('WHPopupContent')&&dgPhonePopups.includes('WHGlossaryAutolink')&&dgPhonePage.indexOf('./phone-popup-controller.js?v=1')<dgPhonePage.indexOf('./mobile.js?v=29')&&sw.includes('"./glossary/generated/glossary.en.js?v=tyranids-1"')&&sw.includes('"./books/death-guard/mobile/phone-popup-controller.js?v=1"')&&sw.includes('"./books/death-guard/mobile/mobile.js?v=29"'));
check('Death Guard datasheets load one shared related rule panel on demand',read('books/death-guard/mobile/mobile.js').includes("fetch('./related-rules.inc?v=4')")&&exists('books/death-guard/mobile/related-rules.inc'));
check('matrix-backed related-rule assets use one current versioned cache contract',read('books/death-guard/scripts/app.js').includes("fetch('./mobile/related-rules.inc?v=4')")&&read('books/death-guard/mobile/mobile.js').includes("fetch('./related-rules.inc?v=4')")&&read('books/adeptus-mechanicus/scripts/app.js').includes("fetch('./mobile/related-rules.inc?v=4')")&&read('books/adeptus-mechanicus/mobile/mobile.js').includes("fetch('./related-rules.inc?v=4')")&&bookHtml('death-guard').includes('./scripts/app.js?v=42')&&bookHtml('adeptus-mechanicus').includes('./scripts/app.js?v=34')&&read('books/death-guard/mobile/index.html').includes('./mobile.js?v=29')&&read('books/tyranids/mobile/index.html').includes('./mobile.js?v=10')&&read('books/tyranids/reader.html').includes('./scripts/app.js?v=8')&&read('books/tau-empire/mobile/index.html').includes('./mobile.js?v=8')&&read('books/tau-empire/reader.html').includes('./scripts/app.js?v=9')&&read('books/adeptus-mechanicus/mobile/index.html').includes('./mobile.js?v=16')&&sw.includes('"./books/death-guard/mobile/related-rules.inc?v=4"')&&sw.includes('"./books/adeptus-mechanicus/mobile/related-rules.inc?v=4"')&&!sw.includes('related-rules.inc?v=3'));
...
check('Published matrix-backed books do not load the legacy matcher',!read('books/adeptus-mechanicus/reader.html').includes('shared/related-rules-matcher.js')&&!read('books/tyranids/reader.html').includes('shared/related-rules-matcher.js')&&!read('books/tau-empire/reader.html').includes('shared/related-rules-matcher.js')&&!read('books/emperors-children/reader.html').includes('shared/related-rules-matcher.js'));
