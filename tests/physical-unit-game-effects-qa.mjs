...
for(const id of ['detachment-cohort-acquisitus','detachment-lords-of-the-forge','detachment-luminen-auto-choir','detachment-cohort-cybernetica','detachment-skitarii-hunter-cohort'])assert.match(amSource,new RegExp(`active\\.has\\('${id}'\\)`));
...
assert.match(dgFilter,/items\[0\]\?\.game\?\.effects/);assert.match(amFilter,/items\[0\]\?\.game\?\.effects/);assert.match(dgSource,/if \(Array\.isArray\(projectedEffects\)\)/);assert.match(amSource,/if\(Array\.isArray\(context\.projectedEffects\)\)/);assert.doesNotMatch(dgFilter,/querySelectorAll|data-rule-facts|DocumentFragment/);assert.doesNotMatch(amFilter,/querySelectorAll|data-rule-facts|DocumentFragment/);
