...
const contentRequirement=blueprint.slice(blueprint.indexOf('## AB-CONTENT-002 '),blueprint.indexOf('## AB-CONTENT-003 '));
...
assert.match(contentRequirement,/Every rendered Stratagem card MUST display its canonical Stratagem type as visible text/, 'AB-CONTENT-002 must require a visible canonical type label');
assert.match(contentRequirement,/machine-readable metadata such as data-stratagem-type alone is insufficient/, 'AB-CONTENT-002 must reject metadata-only type presentation');
assert.match(contentRequirement,/exactly one visible type label/, 'AB-CONTENT-002 must require label cardinality across rendering paths');
assert.match(contentRequirement,/MUST visibly state that the type is unverified/, 'AB-CONTENT-002 must preserve honest unknown type presentation');
assert.match(contentRequirement,/Turn timing MUST remain separate from the type label/, 'AB-CONTENT-002 must separate timing from type color semantics');
