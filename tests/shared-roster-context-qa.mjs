...
vm.runInNewContext(fs.readFileSync(path.join(root,'books/shared/roster-parser.js'),'utf8'),sandbox,{filename:'roster-parser.js'});
...
const renamedCatalog={...catalog,units:[{...catalog.units[0],title:'Renamed Example Unit'}]},storedRecord={id:'saved-identity',sourceText:'FACTION KEYWORD: Fixture Faction\nDETACHMENT: Alpha\n1x Example Unit (100 pts)',roster:{faction:'Fixture Faction',units:[{id:'parsed-unit-1',canonicalUnitId:'unit-example',name:'Example Unit'}]}};
const renamedProjection=api.fromRuntime({force:true,catalog:renamedCatalog,bookId:'fixture',location:{search:'?roster=saved-identity'},storage:{getItem:()=>JSON.stringify([storedRecord])}});
assert.equal(renamedProjection.status,'ready','stored canonical Datasheet identity did not survive a display-title change');
assert.equal(renamedProjection.units[0].canonicalDatasheetId,'unit-example','display-title change replaced stable canonical identity');
const ambiguousCatalog={...catalog,units:[...catalog.units,{...catalog.units[0],id:'unit-example-2'}]};
assert.equal(api.project({catalog:ambiguousCatalog,roster:{faction:'Fixture Faction',units:[{id:'physical-ambiguous',name:'Example Unit'}],detachments:[]},record:{id:'ambiguous'}}).context.status,'unknown','ambiguous title resolved to an arbitrary canonical Datasheet');
