...
const migratedBooks=new Map([['death-guard',48],['adeptus-mechanicus',47],['tau-empire',49],['tyranids',63],['emperors-children',36],['chaos-space-marines',74],['space-marines',127],['dark-angels',125],['blood-angels',124]]);
...
for(const file of ['books/tyranids/scripts/app.js','books/tau-empire/scripts/app.js','books/emperors-children/scripts/app.js','books/chaos-space-marines/scripts/app.js']){
  const source=read(file);
...
assert.match(sharedPresentation,/typeNodes=\[\.\.\.card\.querySelectorAll\('\.stratagem-type'\)\]/,'shared decorator must inventory existing labels');
...
for(const file of ['books/tyranids/scripts/app.js','books/tau-empire/scripts/app.js','books/emperors-children/scripts/app.js','books/chaos-space-marines/scripts/app.js'])assert.match(read(file),/shared\/stratagem-presentation\.mjs/,`${file}: shared Stratagem presentation is absent`);
