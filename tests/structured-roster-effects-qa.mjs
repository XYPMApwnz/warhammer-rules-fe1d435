...
try{
  ...
  try{
    ...
    for(const [bookId,{catalog,unit,enhancement,detachment}] of fixtures){
      ...
      assert.match(state.text,new RegExp(enhancement.title.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'i'),`${bookId}: provenance title`);
      assert.ok(state.changes>0,`${bookId}: dynamic roster presentation`);
