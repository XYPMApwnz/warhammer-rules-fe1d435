...
const relation=instanceId=>({instanceId,certainty:'current',provenance:{kind:'explicit-roster-attachment'}});
const attach=(body,...leaders)=>{body.attachments.leaders=leaders.map(leader=>relation(leader.identity.instanceId));for(const leader of leaders)leader.attachments.leading=[relation(body.identity.instanceId)];};
...
const precisionId='enhancement-precision-of-the-patient-hunter',precisionOwner=draft('precision-1','unit-cadre-fireblade',{detachments:['kauyon']}),precisionResolution={input:{ownerStatus:'resolved',ownerUnitId:'precision-1'},catalog:catalog.enhancements.find(item=>item.id===precisionId)};const precisionEffects=effects(precisionOwner,[precisionOwner],[precisionResolution]);assert.ok(precisionEffects.some(item=>item.canonicalReference?.id===precisionId));assert.equal(precisionEffects.some(item=>item.operation!=='reference'),false,'Precision live roll modifiers are not profile mutations');
const kroothawkId='enhancement-kroothawk-flock',shaper=draft('shaper-1','unit-kroot-war-shaper',{detachments:['kroot-hunting-pack']}),carnivores=draft('kroot-1','unit-kroot-carnivores',{detachments:['kroot-hunting-pack']});attach(carnivores,shaper);const kroothawkResolution={input:{ownerStatus:'resolved',ownerUnitId:'shaper-1'},catalog:catalog.enhancements.find(item=>item.id===kroothawkId)};const krootEffects=effects(carnivores,[shaper,carnivores],[kroothawkResolution]);assert.ok(has(krootEffects,'kroothawk-cover'));assert.ok(krootEffects.some(item=>item.canonicalReference?.id===kroothawkId),'Class C full canonical reference');
